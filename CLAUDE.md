...
- **`data/`** → private repo `mrpmohiburrahman/kg-data` (Git LFS). Big DB blobs: `programming-docs.db` (read by `backend/scripts/seed_catalog.py` + `kg-pipeline/build_manifest.py`), `context7.db`, `context7.sql.xz`. Never commit big binaries anywhere else.
...
`data/` uses LFS — run `git -C data lfs pull` after a fresh clone to hydrate the blobs.
