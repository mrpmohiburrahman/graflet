...
For simple redirects, consider using the [`redirects`](../03-api-reference/05-config/01-next-config-js/redirects.mdx) configuration in `next.config.ts` first. Proxy should be used when you need access to request data or more complex logic.

Proxy is _not_ intended for slow data fetching. While Proxy can be helpful for [optimistic checks](../02-guides/authentication.mdx#optimistic-checks-with-proxy-optional) such as permission-based redirects, it should not be used as a full session management or authorization solution.
...
The `matcher` config allows you to filter Proxy to run on specific paths. See the [Matcher](../03-api-reference/03-file-conventions/proxy.mdx#matcher) documentation for more details on path matching.

Read more about [using `proxy`](../02-guides/backend-for-frontend.mdx#proxy), or refer to the `proxy` [API reference](../03-api-reference/03-file-conventions/proxy.mdx).
