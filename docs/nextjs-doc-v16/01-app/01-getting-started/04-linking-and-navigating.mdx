...
In Next.js, [Layouts and Pages](03-layouts-and-pages.mdx) are [React Server Components](https://react.dev/reference/rsc/server-components) by default. On initial and subsequent navigations, the [Server Component Payload](05-server-and-client-components.mdx#how-do-server-and-client-components-work-in-nextjs) is generated on the server before being sent to the client.

There are two types of server rendering, based on _when_ it happens:

- **Prerendering** happens at build time or during [revalidation](09-revalidating.mdx) and the result is cached.
...
Next.js automatically prefetches routes linked with the [`<Link>` component](../03-api-reference/02-components/link.mdx) when they enter the user's viewport.
...
- **Dynamic Route**: prefetching is skipped, or the route is partially prefetched if [`loading.tsx`](../03-api-reference/03-file-conventions/loading.mdx) is present.
...
Next.js also handles [scrolling to the top of the page](../03-api-reference/02-components/link.mdx#scroll) during client-side transitions. If content scrolls behind a sticky or fixed header after navigation, you can fix this with CSS [`scroll-padding-top`](../03-api-reference/02-components/link.mdx#scroll-offset-with-sticky-headers).
...
> **Good to know**: In development mode, you can use the Next.js Devtools to identify if the route is static or dynamic. See [`devIndicators`](../03-api-reference/05-config/01-next-config-js/devIndicators.mdx) for more information.

### Dynamic segments without `generateStaticParams`

If a [dynamic segment](../03-api-reference/03-file-conventions/dynamic-routes.mdx) could be prerendered but isn't because it's missing [`generateStaticParams`](../03-api-reference/04-functions/generate-static-params.mdx), the route will fallback to dynamic rendering at request time.
...
To improve perceived performance, you can use the [`useLinkStatus` hook](../03-api-reference/04-functions/use-link-status.mdx) to show immediate feedback while the transition is in progress.
...
- Using the [`@next/bundle-analyzer`](../02-guides/package-bundling.mdx#nextbundle-analyzer-for-webpack) plugin to identify and reduce bundle size by removing large dependencies.
- Moving logic from the client to the server where possible. See the [Server and Client Components](05-server-and-client-components.mdx) docs for guidance.
...
`pushState` and `replaceState` calls integrate into the Next.js Router, allowing you to sync with [`usePathname`](../03-api-reference/04-functions/use-pathname.mdx) and [`useSearchParams`](../03-api-reference/04-functions/use-search-params.mdx).
