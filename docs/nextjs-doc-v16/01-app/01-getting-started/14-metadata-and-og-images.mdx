...
3. Special [file conventions](../03-api-reference/03-file-conventions/01-metadata/index.mdx) that can be used to add static or dynamically generated [favicons](#favicons) and [OG images](#static-open-graph-images).
...
To define static metadata, export a [`Metadata` object](../03-api-reference/04-functions/generate-metadata.mdx#metadata-object) from a static [`layout.js`](../03-api-reference/03-file-conventions/layout.mdx) or [`page.js`](../03-api-reference/03-file-conventions/page.mdx) file. For example, to add a title and description to the blog route:
...
You can view a full list of available options, in the [`generateMetadata` documentation](../03-api-reference/04-functions/generate-metadata.mdx#metadata-fields).

## Generated metadata

You can use [`generateMetadata`](../03-api-reference/04-functions/generate-metadata.mdx) function to `fetch` metadata that depends on data. For example, to fetch the title and description for a specific blog post:
...
You can customize or **disable** streaming metadata completely, with the [`htmlLimitedBots`](../03-api-reference/05-config/01-next-config-js/htmlLimitedBots.mdx#disabling) option in your Next.js config file.

Prerendered pages don’t use streaming since metadata is resolved at build time.

Learn more about [streaming metadata](../03-api-reference/04-functions/generate-metadata.mdx#streaming-metadata).
...
- [favicon.ico, apple-icon.jpg, and icon.jpg](../03-api-reference/03-file-conventions/01-metadata/app-icons.mdx)
- [opengraph-image.jpg and twitter-image.jpg](../03-api-reference/03-file-conventions/01-metadata/opengraph-image.mdx)
...
> You can also programmatically generate favicons using code. See the [favicon docs](../03-api-reference/03-file-conventions/01-metadata/app-icons.mdx) for more information.
...
> Other image formats such as `jpeg`, `png`, and `gif` are also supported. See the [Open Graph Image docs](../03-api-reference/03-file-conventions/01-metadata/opengraph-image.mdx) for more information.

## Generated Open Graph images

The [`ImageResponse` constructor](../03-api-reference/04-functions/image-response.mdx) allows you to generate dynamic images using JSX and CSS. This is useful for OG images that depend on data.
...
`ImageResponse` supports common CSS properties including flexbox and absolute positioning, custom fonts, text wrapping, centering, and nested images. [See the full list of supported CSS properties](../03-api-reference/04-functions/image-response.mdx).
