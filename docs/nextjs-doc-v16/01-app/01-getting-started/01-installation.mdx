...
Learn more about [browser support](../../03-architecture/supported-browsers.mdx), including how to configure polyfills and target specific browsers.

## Create with the CLI

The quickest way to create a new Next.js app is using [`create-next-app`](../03-api-reference/06-cli/create-next-app.mdx), which sets up everything automatically for you. To create a project, run:
...
After the prompts, [`create-next-app`](../03-api-reference/06-cli/create-next-app.mdx) will create a folder with your project name and install the required dependencies.
...
Turbopack is now the default bundler. To use Webpack run `next dev --webpack` or `next build --webpack`. See the [Turbopack docs](../03-api-reference/08-turbopack.mdx) for configuration details.
...
Create an `app` folder. Then, inside `app`, create a `layout.tsx` file. This file is the [root layout](../03-api-reference/03-file-conventions/layout.mdx#root-layout). It's required and must contain the `<html>` and `<body>` tags.
...
> - You can optionally use a [`src` folder](../03-api-reference/03-file-conventions/src-folder.mdx) in the root of your project to separate your application's code from configuration files.
...
Next, add an `_app.tsx` file inside `pages/` to define the global layout. Learn more about the [custom App file](../../02-pages/03-building-your-application/01-routing/05-custom-app.mdx).
...
Finally, add a `_document.tsx` file inside `pages/` to control the initial response from the server. Learn more about the [custom Document file](../../02-pages/03-building-your-application/01-routing/06-custom-document.mdx).
...
Create a [`public` folder](../03-api-reference/03-file-conventions/public-folder.mdx) at the root of your project to store static assets such as images, fonts, etc. Files inside `public` can then be referenced by your code starting from the base URL (`/`).
...
See the [TypeScript reference](../03-api-reference/05-config/02-typescript.mdx) page for more information.
...
See the [ESLint Plugin](../03-api-reference/05-config/03-eslint.mdx) page for more information.
