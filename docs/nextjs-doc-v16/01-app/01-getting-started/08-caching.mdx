...
> This page covers caching with [Cache Components](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx), enabled by setting [`cacheComponents: true`](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) in your `next.config.ts` file. If you're not using Cache Components, see the [Caching and Revalidating (Previous Model)](../02-guides/caching-without-cache-components.mdx) guide.
...
You can enable Cache Components by adding the [`cacheComponents`](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) option to your Next config file:
...
> **Good to know:** When Cache Components is enabled, `GET` Route Handlers follow the same prerendering model as pages. See [Route Handlers with Cache Components](15-route-handlers.mdx#with-cache-components) for details.

## Usage

The [`use cache`](../03-api-reference/01-directives/use-cache.mdx) directive caches the return value of async functions and components. You can apply it at two levels:
...
> Arguments and any closed-over values from parent scopes automatically become part of the [cache key](../03-api-reference/01-directives/use-cache.mdx#cache-keys), which means different inputs will produce separate cache entries. This enables personalized or parameterized cached content. See [serialization requirements and constraints](../03-api-reference/01-directives/use-cache.mdx#constraints) for details on what can be cached and how arguments work.
...
- [`cookies`](../03-api-reference/04-functions/cookies.mdx) - User's cookie data
- [`headers`](../03-api-reference/04-functions/headers.mdx) - Request headers
- [`searchParams`](../03-api-reference/03-file-conventions/page.mdx#searchparams-optional) - URL query parameters
- [`params`](../03-api-reference/03-file-conventions/page.mdx#params-optional) - Dynamic route parameters (unless at least one sample is provided via [`generateStaticParams`](../03-api-reference/04-functions/generate-static-params.mdx)).
...
By default, `use cache` stores entries [in-memory](../03-api-reference/01-directives/use-cache.mdx#runtime-caching-considerations). In serverless environments where memory doesn't persist across requests, `CachedContent` may re-evaluate on every request. Consider [`'use cache: remote'`](../03-api-reference/01-directives/use-cache-remote.mdx) for durable, shared caching.
...
**To generate unique values per request**, defer to request time by calling [`connection()`](../03-api-reference/04-functions/connection.mdx) before these operations, and wrap the component in `<Suspense>`:
...
> **Good to know:** This includes queries to embedded databases with synchronous APIs, such as `better-sqlite3` or Node.js's built-in [`node:sqlite`](https://nodejs.org/api/sqlite.html). If you need per-request data from a synchronous source, call [`connection()`](../03-api-reference/04-functions/connection.mdx) before the query.
...
This generates a static shell consisting of HTML for initial page loads and a serialized [RSC Payload](05-server-and-client-components.mdx#on-the-server) for client-side navigation, ensuring the browser receives fully rendered content instantly whether users navigate directly to the URL or transition from another page.
...
> You can verify that a route was fully prerendered by checking the [build output summary](../03-api-reference/06-cli/next.mdx#next-build-options). Alternatively, see what content was added to the static shell of any page by viewing the page source in your browser.
...
Placing a `<Suspense>` boundary with an empty fallback above the document body in your Root Layout causes the entire app to defer to request time. Because the fallback is empty, there is no static shell to send immediately, so every request blocks until the page is fully rendered. To limit this to specific routes, use [multiple root layouts](../03-api-reference/03-file-conventions/layout.mdx#root-layout).
...
> **Good to know**: This same pattern applies when `generateViewport` accesses uncached dynamic data. See [Viewport with Cache Components](../03-api-reference/04-functions/generate-viewport.mdx#with-cache-components) for a detailed example.
...
During prerendering, the header (static) and blog posts (cached with `use cache`) become part of the static shell along with the fallback UI for user preferences. Only the personalized preferences stream in at request time. When an admin publishes a new post, the [`updateTag`](09-revalidating.mdx#updatetag) call immediately expires the blog posts cache so the next visitor sees it.

> **Good to know:** `generateMetadata` and `generateViewport` track runtime data access separately from the page. See [Metadata with Cache Components](../03-api-reference/04-functions/generate-metadata.mdx#with-cache-components) and [Viewport with Cache Components](../03-api-reference/04-functions/generate-viewport.mdx#with-cache-components) for how to handle this.
