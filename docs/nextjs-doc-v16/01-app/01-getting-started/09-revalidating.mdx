...
> This page covers revalidation with [Cache Components](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx), enabled by setting [`cacheComponents: true`](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) in your `next.config.ts` file. If you're not using Cache Components, see the [Caching and Revalidating (Previous Model)](../02-guides/caching-without-cache-components.mdx) guide.
...
[`cacheLife`](../03-api-reference/04-functions/cacheLife.mdx) controls how long cached data remains valid. Use it inside a [`use cache`](../03-api-reference/01-directives/use-cache.mdx) scope to set the cache lifetime.
...
> **Good to know:** A cache is considered "short-lived" when it uses the `seconds` profile, `revalidate: 0`, or `expire` under 5 minutes. Short-lived caches are automatically excluded from prerenders and become dynamic holes instead. See [Prerendering behavior](../03-api-reference/04-functions/cacheLife.mdx#prerendering-behavior) for details.

See the [`cacheLife` API reference](../03-api-reference/04-functions/cacheLife.mdx) for all profiles and custom configuration options.

## `cacheTag`

[`cacheTag`](../03-api-reference/04-functions/cacheTag.mdx) lets you tag cached data so it can be invalidated on-demand. Use it inside a [`use cache`](../03-api-reference/01-directives/use-cache.mdx) scope:
...
See the [`cacheTag` API reference](../03-api-reference/04-functions/cacheTag.mdx) to learn more.
...
You can reuse the same tag in multiple functions to revalidate them all at once. Call `revalidateTag` in a [Server Action](07-mutating-data.mdx) or [Route Handler](../03-api-reference/03-file-conventions/route.mdx).
...
See the [`revalidateTag` API reference](../03-api-reference/04-functions/revalidateTag.mdx) to learn more.

## `updateTag`

`updateTag` immediately expires cached data for read-your-own-writes scenarios — the user sees their change right away instead of stale content. Unlike `revalidateTag`, it can only be used in [Server Actions](07-mutating-data.mdx).
...
See the [`updateTag` API reference](../03-api-reference/04-functions/updateTag.mdx) to learn more.
...
See the [`revalidatePath` API reference](../03-api-reference/04-functions/revalidatePath.mdx) to learn more.

## What should I cache?

Cache data that doesn't depend on [runtime data](08-caching.mdx#working-with-runtime-apis) and that you're OK serving from cache for a period of time. Use `use cache` with `cacheLife` to describe that behavior.
...
> **Good to know:** In serverless environments, in-memory cache entries may not persist across revalidations. See [runtime caching considerations](../03-api-reference/01-directives/use-cache.mdx#runtime-caching-considerations) for details.
