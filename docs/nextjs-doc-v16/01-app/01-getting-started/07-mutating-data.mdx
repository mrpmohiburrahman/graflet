...
> [!WARNING]
> Server Functions are reachable via direct POST requests, not just through your application's UI. Always verify authentication and authorization inside every Server Function. See the [Data Security guide](../02-guides/data-security.mdx#authentication-and-authorization) for recommended patterns.
...
> **Good to know:** Server Functions are designed for server-side mutations. The client currently dispatches and awaits them one at a time. This is an implementation detail and may change. If you need parallel data fetching, use [data fetching](06-fetching-data.mdx#server-components) in Server Components, or perform parallel work inside a single Server Function or [Route Handler](../02-guides/backend-for-frontend.mdx#manipulating-data).
...
After a mutation, you may want to refresh the current page to show the latest data. You can do this by calling [`refresh`](../03-api-reference/04-functions/refresh.mdx) from `next/cache` in a Server Action:
...
This refreshes the client router, ensuring the UI reflects the latest state. The `refresh()` function does not revalidate tagged data. To revalidate tagged data, use [`updateTag`](../03-api-reference/04-functions/updateTag.mdx) or [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx) instead.

### Revalidate data

After performing a mutation, you can revalidate the Next.js cache and show the updated data by calling [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx) or [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx) within the Server Function:
...
You may want to redirect the user to a different page after a mutation. You can do this by calling [`redirect`](../03-api-reference/04-functions/redirect.mdx) within the Server Function.
...
Calling `redirect` [throws](../03-api-reference/04-functions/redirect.mdx#behavior) a framework handled control-flow exception. Any code after it won't execute. If you need fresh data, call [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx) or [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx) beforehand.

### Cookies

You can `get`, `set`, and `delete` cookies inside a Server Action using the [`cookies`](../03-api-reference/04-functions/cookies.mdx) API.

When you [set or delete](../03-api-reference/04-functions/cookies.mdx#understanding-cookie-behavior-in-server-functions) a cookie in a Server Action, Next.js re-renders the current page and its layouts on the server so the **UI reflects the new cookie value**.
