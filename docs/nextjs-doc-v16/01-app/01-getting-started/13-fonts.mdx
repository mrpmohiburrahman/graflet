...
The [`next/font`](../03-api-reference/02-components/font.mdx) module automatically optimizes your fonts and removes external network requests for improved privacy and performance.
...
Fonts are scoped to the component they're used in. To apply a font to your entire application, add it to the [Root Layout](../03-api-reference/03-file-conventions/layout.mdx#root-layout).

</AppOnly>

<PagesOnly>

To start using `next/font`, import it from [`next/font/local`](#local-fonts) or [`next/font/google`](#google-fonts), call it as a function with the appropriate options, and set the `className` of the element you want to apply the font to. For example, you can apply fonts globally in your [Custom App](../../02-pages/03-building-your-application/01-routing/05-custom-app.mdx) (`pages/_app`):
...
To use a local font, import the `localFont` function from `next/font/local` and specify the [`src`](../03-api-reference/02-components/font.mdx#src) of your local font file. The path is resolved relative to the file where `localFont` is called. Fonts can be stored anywhere in the project, including the [`public`](../03-api-reference/03-file-conventions/public-folder.mdx) folder or co-located inside the `app` folder. For example, to use a font stored in `app/fonts/`:
...
To use a local font, import your font from `next/font/local` and specify the [`src`](../../02-pages/04-api-reference/01-components/font.mdx#src) of your local font file. Fonts can be stored in the [`public`](../../02-pages/04-api-reference/02-file-conventions/public-folder.mdx) folder or inside the `pages` folder. For example:
