...
Expected errors are those that can occur during the normal operation of the application, such as those from [server-side form validation](../02-guides/forms.mdx) or failed requests. These errors should be handled explicitly and returned to the client.
...
When fetching data inside of a Server Component, you can use the response to conditionally render an error message or [`redirect`](../03-api-reference/04-functions/redirect.mdx).
...
You can call the [`notFound`](../03-api-reference/04-functions/not-found.mdx) function within a route segment and use the [`not-found.js`](../03-api-reference/03-file-conventions/not-found.mdx) file to show a 404 UI.
...
Create an error boundary by adding an [`error.js`](../03-api-reference/03-file-conventions/error.mdx) file inside a route segment and exporting a React component:
...
Errors will bubble up to the nearest parent error boundary. This allows for granular error handling by placing `error.tsx` files at different levels in the [route hierarchy](02-project-structure.mdx#component-hierarchy).
...
For component-level error recovery, the [`unstable_catchError`](../03-api-reference/04-functions/catchError.mdx) function lets you create error boundaries that can wrap any part of your component tree:
...
While less common, you can handle errors in the root layout using the [`global-error.js`](../03-api-reference/03-file-conventions/error.mdx#global-error) file, located in the root app directory, even when leveraging [internationalization](../02-guides/internationalization.mdx). Global error UI must define its own `<html>` and `<body>` tags, since it is replacing the root layout or template when active.
