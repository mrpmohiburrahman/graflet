...
For Next.js-specific Server Action behaviors (response model, security, configuration, deployment), see [Server Actions and Mutations](../../02-guides/server-actions.mdx).
...
Server Function return values are serialized and sent to the client. Only return data the UI needs, not raw database records. See the [Data Security guide](../../02-guides/data-security.mdx#controlling-return-values) for more details.
