...
To use `'use cache: remote'`, enable the [`cacheComponents`](../05-config/01-next-config-js/cacheComponents.mdx) flag in your `next.config.ts` file:
...
Then add `'use cache: remote'` to the functions or components where you've determined remote caching is justified. The handler implementation is configured via [`cacheHandlers`](../05-config/01-next-config-js/cacheHandlers.mdx), though hosting providers should typically provide this automatically. If you're self-hosting, see the `cacheHandlers` configuration reference to set up your cache storage.
...
Remote caching provides the most value when content is deferred to request time (outside the static shell). This typically happens when a component accesses request values like [`cookies()`](../04-functions/cookies.mdx), [`headers()`](../04-functions/headers.mdx), or [`searchParams`](../03-file-conventions/page.mdx#searchparams-optional), placing it inside a Suspense boundary. In this context:
...
Both `use cache` and `'use cache: remote'` can't access runtime values like cookies or search params directly. You can extract these values and pass them as arguments to cached functions. See [with runtime data](../../01-getting-started/08-caching.mdx#working-with-runtime-apis) for this pattern.
...
Only use [`'use cache: private'`](use-cache-private.mdx) if you have compliance requirements or can't refactor to pass runtime data as arguments.
...
The following examples demonstrate common patterns for using `'use cache: remote'`. For details about `cacheLife` parameters (`stale`, `revalidate`, `expire`), see the [`cacheLife` API reference](../04-functions/cacheLife.mdx).
...
Cache expensive database queries, reducing load on your database. In this example, we don't access `cookies()`, `headers()`, or `searchParams`. If we had a requirement to not include these stats in the static shell, we could use [`connection()`](../04-functions/connection.mdx) to explicitly defer to request time:
...
> - `'use cache: remote'` works outside the static shell where [`use cache`](use-cache.mdx) may not provide server-side cache hits
> - Use [`cacheTag()`](../04-functions/cacheTag.mdx) and [`revalidateTag()`](../04-functions/revalidateTag.mdx) to invalidate remote caches on-demand
> - Use [`cacheLife()`](../04-functions/cacheLife.mdx) to configure cache expiration
> - For user-specific data, use [`'use cache: private'`](use-cache-private.mdx) instead of `'use cache: remote'`
...
| [Adapters](../../01-getting-started/17-deploying.mdx#adapters)            | Yes       |
