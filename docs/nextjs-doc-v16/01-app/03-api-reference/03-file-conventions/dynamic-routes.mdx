...
Dynamic Segments are passed as the `params` prop to [`layout`](layout.mdx), [`page`](page.mdx), [`route`](route.mdx), and [`generateMetadata`](../04-functions/generate-metadata.mdx#generatemetadata-function) functions.
...
Alternatively Client Components can use the [`useParams`](../04-functions/use-params.mdx) hook to access the `params` anywhere in the Client Component tree.
...
When using TypeScript, you can add types for `params` depending on your configured route segment — use [`PageProps<'/route'>`](page.mdx#page-props-helper), [`LayoutProps<'/route'>`](layout.mdx#layout-props-helper), or [`RouteContext<'/route'>`](route.mdx#route-context-helper) to type `params` in `page`, `layout`, and `route` respectively.
...
When using [Cache Components](../../01-getting-started/08-caching.mdx) with dynamic route segments, how you handle params depends on whether you use [`generateStaticParams`](../04-functions/generate-static-params.mdx).
...
The [`generateStaticParams`](../04-functions/generate-static-params.mdx) function can be used to [statically generate](../../04-glossary.mdx#prerendering) routes at build time instead of on-demand at request time.
...
When using `fetch` inside the `generateStaticParams` function, the requests are [automatically deduplicated](../../04-glossary.mdx#memoization). This avoids multiple network calls for the same data Layouts, Pages, and other `generateStaticParams` functions, speeding up build time.
