...
The `unstable_instant` route segment config opts a route into validation for instant client-side navigations. Next.js checks, during development and at build time, that the caching structure produces an instant [static shell](../../../04-glossary.mdx#static-shell) at every possible entry point into the route.

> **Good to know**:
>
> - The `unstable_instant` export only works when [`cacheComponents`](../../05-config/01-next-config-js/cacheComponents.mdx) is enabled.
...
The `@next/playwright` package exports an `instant()` helper that holds back dynamic content while the callback runs against the static shell. See the [guide](../../../02-guides/instant-navigation.mdx#prevent-regressions-with-e2e-tests) for a full example.
