...
> - For component-level error recovery that aren't tied to route segments like [`error.js`](error.mdx), use the [`unstable_catchError`](../04-functions/catchError.mdx) function.

In the [component hierarchy](../../01-getting-started/02-project-structure.mdx#component-hierarchy), `error.js` wraps `loading.js`, `not-found.js`, `page.js`, and nested `layout.js` files in a React error boundary. It does **not** wrap the `layout.js` or `template.js` above it in the same segment. To handle errors in the root layout, use [`global-error.js`](error.mdx#global-error).
...
While less common, you can handle errors in the root layout or template using `global-error.jsx`, located in the root app directory, even when leveraging [internationalization](../../02-guides/internationalization.mdx). Global error UI must define its own `<html>` and `<body>` tags, global styles, fonts, or other dependencies that your error page requires. This file replaces the root layout or template when active.

> **Good to know**: Error boundaries must be [Client Components](../../01-getting-started/05-server-and-client-components.mdx#using-client-components), which means that [`metadata` and `generateMetadata`](../../01-getting-started/14-metadata-and-og-images.mdx) exports are not supported in `global-error.jsx`. As an alternative, you can use the React [`<title>`](https://react.dev/reference/react-dom/components/title) component.
