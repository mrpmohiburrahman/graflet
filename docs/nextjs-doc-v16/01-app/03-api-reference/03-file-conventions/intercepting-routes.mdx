...
> **Good to know:** The `(..)` convention is based on _route segments_, not the file-system. For example, it does not consider `@slot` folders in [Parallel Routes](parallel-routes.mdx).

## Examples

### Modals

Intercepting Routes can be used together with [Parallel Routes](parallel-routes.mdx) to create modals. This allows you to solve common challenges when building modals, such as:
...
See the [Parallel Routes](parallel-routes.mdx#modals) documentation for a step-by-step example, or see our [image gallery example](https://github.com/vercel-labs/nextgram).
