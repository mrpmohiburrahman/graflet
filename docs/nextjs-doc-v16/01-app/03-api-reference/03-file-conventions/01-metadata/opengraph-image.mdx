...
> **Good to know**:
>
> - By default, generated images are [**statically optimized**](../../../04-glossary.mdx#prerendering) (generated at build time and cached) unless they use [Request-time APIs](../../../04-glossary.mdx#request-time-apis) or uncached data.
> - You can generate multiple Images in the same file using [`generateImageMetadata`](../../04-functions/generate-image-metadata.mdx).
> - `opengraph-image.js` and `twitter-image.js` are special Route Handlers that is cached by default unless it uses a [Request-time API](../../../04-glossary.mdx#request-time-apis) or [dynamic config](../../../02-guides/caching-without-cache-components.mdx#dynamic) option.

The easiest way to generate an image is to use the [ImageResponse](../../04-functions/image-response.mdx) API from `next/og`.
...
A promise that resolves to an object containing the [dynamic route parameters](../dynamic-routes.mdx) object from the root segment down to the segment `opengraph-image` or `twitter-image` is colocated in.

> **Good to know**: If you use [`generateImageMetadata`](../../04-functions/generate-image-metadata.mdx), the function will also receive an `id` prop that is a promise resolving to the `id` value from one of the items returned by `generateImageMetadata`.
...
`opengraph-image` and `twitter-image` are specialized [Route Handlers](../route.mdx) that can use the same [route segment configuration](../02-route-segment-config/index.mdx) options as Pages and Layouts.
...
> **Good to know**:
> By default, this generated image will be statically optimized. You can configure the individual `fetch` [`options`](../../04-functions/fetch.mdx) or route segments [options](../../../02-guides/caching-without-cache-components.mdx#route-segment-config-revalidate) to change this behavior.
