...
The easiest way to generate an icon is to use the [`ImageResponse`](../../04-functions/image-response.mdx) API from `next/og`.
...
> **Good to know**:
>
> - By default, generated icons are [**statically optimized**](../../../04-glossary.mdx#prerendering) (generated at build time and cached) unless they use [Request-time APIs](../../../04-glossary.mdx#request-time-apis) or uncached data.
> - You can generate multiple icons in the same file using [`generateImageMetadata`](../../04-functions/generate-image-metadata.mdx).
> - You cannot generate a `favicon` icon. Use [`icon`](#icon) or a [favicon.ico](#favicon) file instead.
> - App icons are special Route Handlers that are cached by default unless they use a [Request-time API](../../../04-glossary.mdx#request-time-apis) or [dynamic config](../../../02-guides/caching-without-cache-components.mdx#dynamic) option.
...
A promise that resolves to an object containing the [dynamic route parameters](../dynamic-routes.mdx) object from the root segment down to the segment `icon` or `apple-icon` is colocated in.
...
`icon` and `apple-icon` are specialized [Route Handlers](../route.mdx) that can use the same [route segment configuration](../02-route-segment-config/index.mdx) options as Pages and Layouts.
