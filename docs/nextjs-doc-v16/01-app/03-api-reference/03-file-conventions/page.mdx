...
- In the [component hierarchy](../../01-getting-started/02-project-structure.mdx#component-hierarchy), `page.js` is the innermost file convention. It is wrapped by `loading.js` (Suspense boundary), `error.js` (error boundary), `template.js`, and `layout.js` in the same segment.
...
A promise that resolves to an object containing the [dynamic route parameters](dynamic-routes.mdx) from the root segment down to that page.
...
- `searchParams` is a **[Request-time API](../../04-glossary.mdx#request-time-apis)** whose values cannot be known ahead of time. Using it will opt the page into **[dynamic rendering](../../04-glossary.mdx#dynamic-rendering)** at request time.
...
Using [dynamic route segments](dynamic-routes.mdx), you can display or fetch specific content for the page based on the `params` prop.
...
| Version      | Changes                                                                                                          |
| ------------ | ---------------------------------------------------------------------------------------------------------------- |
| `v15.0.0-RC` | `params` and `searchParams` are now promises. A [codemod](../../02-guides/upgrading/codemods.mdx#150) is available. |
