...
- **`not-found.js`**: Used when you call the [`notFound`](../04-functions/not-found.mdx) function in a route segment.
...
The **not-found** file is used to render UI when the [`notFound`](../04-functions/not-found.mdx) function is thrown within a route segment. Along with serving a custom UI, Next.js will return a `200` HTTP status code for streamed responses, and `404` for non-streamed responses (see [Status Codes](loading.mdx#status-codes) for details about SEO).
...
In the [component hierarchy](../../01-getting-started/02-project-structure.mdx#component-hierarchy), `not-found.js` renders between `loading.js` and `page.js`. It is wrapped by the `<Suspense>` boundary from `loading.js` and the error boundary from `error.js` in the same segment.
...
For `global-not-found.js`, you can export a `metadata` object or a [`generateMetadata`](../04-functions/generate-metadata.mdx) function to customize the `<title>`, `<meta>`, and other head tags for your 404 page:
