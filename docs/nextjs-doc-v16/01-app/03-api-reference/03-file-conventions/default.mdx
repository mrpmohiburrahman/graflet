...
The `default.js` file is used to render a fallback within [Parallel Routes](parallel-routes.mdx) when Next.js cannot recover a [slot's](parallel-routes.mdx#slots) active state after a full-page load.

During [soft navigation](../../01-getting-started/04-linking-and-navigating.mdx#client-side-transitions), Next.js keeps track of the active _state_ (subpage) for each slot. However, for hard navigations (full-page load), Next.js cannot recover the active state. In this case, a `default.js` file can be rendered for subpages that don't match the current URL.
...
A promise that resolves to an object containing the [dynamic route parameters](dynamic-routes.mdx) from the root segment down to the slot's subpages. For example:
