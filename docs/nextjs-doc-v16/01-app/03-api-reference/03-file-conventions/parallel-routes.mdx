...
However, slots are **not** route segments and do not affect the URL structure. For example, for `/@analytics/views`, the URL will be `/views` since `@analytics` is a slot. Slots are combined with the regular [Page](page.mdx) component to form the final page associated with the route segment. Because of this, you cannot have separate [prerendered](../../04-glossary.mdx#prerendering) and [dynamically rendered](../../04-glossary.mdx#dynamic-rendering) slots at the same route segment level. If one slot is dynamic, all slots at that level must be dynamic.
...
- [**Soft Navigation**](../../01-getting-started/04-linking-and-navigating.mdx#client-side-transitions): During client-side navigation, Next.js will perform a [partial render](../../01-getting-started/04-linking-and-navigating.mdx#client-side-transitions), changing the subpage within the slot, while maintaining the other slot's active subpages, even if they don't match the current URL.
...
Both [`useSelectedLayoutSegment`](../04-functions/use-selected-layout-segment.mdx) and [`useSelectedLayoutSegments`](../04-functions/use-selected-layout-segments.mdx) accept a `parallelRoutesKey` parameter, which allows you to read the active route segment within a slot.
...
Within `@analytics`, create a [`layout`](layout.mdx) file to share the tabs between the two pages:
...
Parallel Routes can be used together with [Intercepting Routes](intercepting-routes.mdx) to create modals that support deep linking. This allows you to solve common challenges when building modals, such as:
...
Then, inside the `@auth` slot, add [`default.js`](default.mdx) file that returns `null`. This ensures that the modal is not rendered when it's not active.
...
> **Good to know:**
>
> - The convention `(.)` is used for intercepting routes. See [Intercepting Routes](intercepting-routes.mdx#convention) docs for more information.
> - By separating the `<Modal>` functionality from the modal content (`<Login>`), you can ensure any content inside the modal, e.g. [forms](../../02-guides/forms.mdx), are Server Components. See [Interleaving Client and Server Components](../../01-getting-started/05-server-and-client-components.mdx#interleaving-server-and-client-components) for more information.
...
See the [Loading UI](loading.mdx) and [Error Handling](../../01-getting-started/10-error-handling.mdx) documentation for more information.
