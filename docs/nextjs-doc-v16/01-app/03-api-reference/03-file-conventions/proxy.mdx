...
The `proxy.js|ts` file is used to write [Proxy](../../01-getting-started/16-proxy.mdx) and run code on the server before a request is completed. Then, based on the incoming request, you can modify the response by rewriting, redirecting, modifying the request or response headers, or responding directly.
...
>
> To pass information from Proxy to your application, use [headers](#setting-headers), [cookies](#using-cookies), [rewrites](../04-functions/next-response.mdx#rewrite), [redirects](../04-functions/next-response.mdx#redirect), or the URL.
...
If you’ve customized [`pageExtensions`](../05-config/01-next-config-js/pageExtensions.mdx), for example to `.page.ts` or `.page.js`, name your file `proxy.page.ts` or `proxy.page.js` accordingly.
...
1. `rewrite` to a route ([Page](../../../02-pages/03-building-your-application/01-routing/01-pages-and-layouts.mdx) or [Edge API Route](../../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx)) that produces a response
2. return a `NextResponse` directly. See [Producing a Response](#producing-a-response)
...
> **Good to know:** [Server Functions](../01-directives/use-server.mdx) are not separate routes in this chain. They are handled as POST requests to the route where they are used, so a Proxy matcher that excludes a path will also skip Server Function calls on that path.
>
> A matcher change or a refactor that moves a Server Function to a different route can silently remove Proxy coverage. Always verify authentication and authorization inside each Server Function rather than relying on Proxy alone. See the [Data Security guide](../../02-guides/data-security.mdx#authentication-and-authorization) for recommended patterns.

## Runtime

Proxy defaults to using the Node.js runtime. The [`runtime`](02-route-segment-config/runtime.mdx) config option is not available in Proxy files. Setting the `runtime` config option in Proxy will throw an error.
...
Learn more in [NextResponse headers in Proxy](../04-functions/next-response.mdx#next).
...
Learn how to [configure Proxy](../../02-guides/self-hosting.mdx#proxy) when self-hosting Next.js.
