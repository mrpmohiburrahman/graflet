...
[manual-loader]: 05-config/01-next-config-js/turbopack.mdx#configuring-webpack-loaders
...
| **Manual Aliases**    | **Supported**       | [Configure `resolveAlias` in `next.config.js`](05-config/01-next-config-js/turbopack.mdx#resolving-aliases) (similar to `webpack.resolve.alias`). |
| **Custom Extensions** | **Supported**       | [Configure `resolveExtensions` in `next.config.js`](05-config/01-next-config-js/turbopack.mdx#resolving-custom-extensions).                       |
...
See [Lazy Loading](../02-guides/lazy-loading.mdx#magic-comments) for usage examples.
...
You can configure the filesystem root using [turbopack.root](05-config/01-next-config-js/turbopack.mdx#root-directory) option in `next.config.js`.

### CSS Module Ordering

Turbopack will follow JS import order to order [CSS modules](../01-getting-started/11-css.mdx#css-modules) which are not otherwise ordered. For example:
...
- [`experimental.turbopackFileSystemCacheForDev`](05-config/01-next-config-js/turbopackFileSystemCache.mdx) is enabled by default
- [`experimental.turbopackFileSystemCacheForBuild`](05-config/01-next-config-js/turbopackFileSystemCache.mdx) is currently opt-in
...
Turbopack does not support webpack plugins. This affects third-party tools that rely on webpack's plugin system for integration. We do support [webpack loaders](05-config/01-next-config-js/turbopack.mdx#configuring-webpack-loaders). If you depend on webpack plugins, you'll need to find Turbopack-compatible alternatives or continue using webpack until equivalent functionality is available.
...
- **`webpack()` configuration** in `next.config.js`
  Turbopack replaces webpack, so `webpack()` configs are not recognized. Use the [`turbopack` config](05-config/01-next-config-js/turbopack.mdx) instead.
- **Yarn PnP**
...
For a full, detailed breakdown of each feature flag and its status, see the [Turbopack API Reference](05-config/01-next-config-js/turbopack.mdx).
...
- **`rules`**
  Define additional [webpack loaders](05-config/01-next-config-js/turbopack.mdx#configuring-webpack-loaders) for file transformations.
- **`resolveAlias`**
  Create manual aliases (like `resolve.alias` in webpack).
- **`resolveExtensions`**
  Change or extend file extensions for module resolution.
- **[`ignoreIssue`](05-config/01-next-config-js/turbopackIgnoreIssue.mdx)**
...
| ------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------- | ------------- | ----------------------------- |
| [`turbopackFileSystemCacheForDev`](05-config/01-next-config-js/turbopackFileSystemCache.mdx)   | Enable filesystem cache for the dev server.                                             | `true`        | N/A                           |
| [`turbopackFileSystemCacheForBuild`](05-config/01-next-config-js/turbopackFileSystemCache.mdx) | Enable filesystem cache for builds.                                                     | N/A           | `false`                       |
...
| [`turbopackLocalPostcssConfig`](05-config/01-next-config-js/turbopackLocalPostcssConfig.mdx)   | Resolve `postcss.config.js` from the CSS file's directory first, then the project root. | `false`       | `false`                       |
...
For more in-depth configuration examples, see the [Turbopack config documentation](05-config/01-next-config-js/turbopack.mdx).
