...
> **Good to know**: If you are using a version of Next.js prior to 13, you'll want to use the [next/legacy/image](../../../02-pages/04-api-reference/01-components/image-legacy.mdx) documentation since the component was renamed.
...
- [Custom Image Loader Configuration](../05-config/01-next-config-js/images.mdx#example-loader-configuration)
...
`minimumCacheTTL` allows you to configure the Time to Live (TTL) in seconds for cached optimized images. In many cases, it's better to use a [Static Image Import](../../01-getting-started/12-images.mdx#local-images) which will automatically hash the file contents and cache the image forever with a `Cache-Control` header of `immutable`.
...
If you need to change the caching behavior per image, you can configure [`headers`](../05-config/01-next-config-js/headers.mdx) to set the `Cache-Control` header on the upstream image (e.g. `/some-asset.jpg`, not `/_next/image` itself).
...
Alternatively, you can implement your own cache handler using [`cacheHandler`](../05-config/01-next-config-js/incrementalCacheHandlerPath.mdx) which will ignore the `maximumDiskCacheSize` configuration.
...
- SVG has many of the same features as HTML/CSS, which can lead to vulnerabilities without proper [Content Security Policy (CSP) headers](../05-config/01-next-config-js/headers.mdx#content-security-policy).
...
Use `className` or `style`, not `styled-jsx`. In most cases, we recommend using the `className` prop. This can be an imported [CSS Module](../../01-getting-started/11-css.mdx), a [global stylesheet](../../01-getting-started/11-css.mdx#global-css), etc.
...
You cannot use [styled-jsx](../../02-guides/css-in-js.mdx) because it's scoped to the current component (unless you mark the style as `global`).
...
Since Next.js does not have access to remote files during the build process, you'll need to provide the [`width`](image.mdx#width-and-height), [`height`](image.mdx#width-and-height) and optional [`blurDataURL`](image.mdx#blurdataurl) props manually.
...
| `v13.0.0`  | The `next/image` import was renamed to `next/legacy/image`. The `next/future/image` import was renamed to `next/image`. A [codemod is available](../../02-guides/upgrading/codemods.mdx#next-image-to-legacy-image) to safely and automatically rename your imports. `<span>` wrapper removed. `layout`, `objectFit`, `objectPosition`, `lazyBoundary`, `lazyRoot` props removed. `alt` is required. `onLoadingComplete` receives reference to `img` element. Built-in loader config removed. |
