...
[`next/font`](font.mdx) automatically optimizes your fonts (including custom fonts) and removes external network requests for improved privacy and performance.
...
To use the font in all your pages, add it to [`_app.js` file](../../../02-pages/03-building-your-application/01-routing/05-custom-app.mdx) under `/pages` as shown below:
...
The font [`subsets`](https://fonts.google.com/knowledge/glossary/subsetting) defined by an array of string values with the names of each subset you would like to be [preloaded](font.mdx#specifying-a-subset). Fonts specified via `subsets` will have a link preload tag injected into the head when the [`preload`](#preload) option is true, which is the default.
...
A boolean value that specifies whether the font should be [preloaded](font.mdx#preloading) or not. The default is `true`.
...
Google Fonts are automatically [subset](https://fonts.google.com/knowledge/glossary/subsetting). This reduces the size of the font file and improves performance. You'll need to define which of these subsets you want to preload. Failing to specify any subsets while [`preload`](font.mdx#preload) is `true` will result in a warning.
...
View the [Font API Reference](font.mdx) for more information.
...
Alternatively, you can create a [CSS variable](font.mdx#variable) and use it with your preferred CSS solution:
...
Finally, add the CSS variable to your [Tailwind CSS config](../../01-getting-started/11-css.mdx#tailwind-css):
...
- If it's a [unique page](../03-file-conventions/page.mdx), it is preloaded on the unique route for that page.
- If it's a [layout](../03-file-conventions/layout.mdx), it is preloaded on all the routes wrapped by the layout.
- If it's the [root layout](../03-file-conventions/layout.mdx#root-layout), it is preloaded on all routes.
...
- if it's a [unique page](../../../02-pages/03-building-your-application/01-routing/01-pages-and-layouts.mdx), it is preloaded on the unique route for that page
- if it's in the [custom App](../../../02-pages/03-building-your-application/01-routing/05-custom-app.mdx), it is preloaded on all the routes of the site under `/pages`
