...
`<Link>` is a React component that extends the HTML `<a>` element to provide [prefetching](../../01-getting-started/04-linking-and-navigating.mdx#prefetching) and client-side navigation between routes. It is the primary way to navigate between routes in Next.js.
...
**Defaults to `true`.** The default scrolling behavior of `<Link>` in Next.js **is to maintain scroll position**, similar to how browsers handle back and forwards navigation. When you navigate to a new [Page](../03-file-conventions/page.mdx), scroll position will stay the same as long as the Page is visible in the viewport. However, if the Page is not visible in the viewport, Next.js will scroll to the top of the first Page element.
...
- **`"auto"` or `null` (default)**: Prefetch behavior depends on whether the route is static or dynamic. For static routes, the full route will be prefetched (including all its data). For dynamic routes, the partial route down to the nearest segment with a [`loading.js`](../03-file-conventions/loading.mdx#instant-loading-states) boundary will be prefetched.
...
- `false`: Prefetching will not happen when entering the viewport, but will happen on hover. If you want to completely remove fetching on hover as well, consider using an `<a>` tag or [incrementally adopting](../../02-guides/migrating/app-router-migration.mdx) the App Router, which enables disabling prefetching on hover too.
...
Update the path of the current page without rerunning [`getStaticProps`](../../../02-pages/03-building-your-application/03-data-fetching/01-get-static-props.mdx), [`getServerSideProps`](../../../02-pages/03-building-your-application/03-data-fetching/03-get-server-side-props.mdx) or [`getInitialProps`](../../../02-pages/04-api-reference/03-functions/get-initial-props.mdx). Defaults to `false`.
...
When linking to [dynamic segments](../03-file-conventions/dynamic-routes.mdx), you can use [template literals and interpolation](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Template_literals) to generate a list of links. For example, to generate a list of blog posts:
...
You can use [`usePathname()`](../04-functions/use-pathname.mdx) to determine if a link is active. For example, to add a class to the active link, you can check if the current `pathname` matches the `href` of the link:
...
For [dynamic route segments](../../../02-pages/03-building-your-application/01-routing/02-dynamic-routes.mdx#convention), it can be handy to use template literals to create the link's path.
...
> **Good to know**:
>
> - Next.js will scroll to the [Page](../03-file-conventions/page.mdx) if it is not visible in the viewport upon navigation.
...
- A [dynamic route](../../../02-pages/03-building-your-application/01-routing/02-dynamic-routes.mdx#convention): `/blog/my-post`
...
The default scrolling behavior of `<Link>` in Next.js **is to maintain scroll position**, similar to how browsers handle back and forwards navigation. When you navigate to a new [Page](../03-file-conventions/page.mdx), scroll position will stay the same as long as the Page is visible in the viewport.
...
It's common to use [Proxy](../03-file-conventions/proxy.mdx) for authentication or other purposes that involve rewriting the user to a different page. In order for the `<Link />` component to properly prefetch links with rewrites via Proxy, you need to tell Next.js both the URL to display and the URL to prefetch. This is required to avoid un-necessary fetches to proxy to know the correct route to prefetch.
...
> **Good to know**: If you're using [Dynamic Routes](../../../02-pages/03-building-your-application/01-routing/02-dynamic-routes.mdx#convention), you'll need to adapt your `as` and `href` props. For example, if you have a Dynamic Route like `/dashboard/authed/[user]` that you want to present differently via proxy, you would write: `<Link href={{ pathname: '/dashboard/authed/[user]', query: { user: username } }} as="/dashboard/[user]">Profile</Link>`.
...
| `v13.0.0` | No longer requires a child `<a>` tag. A [codemod](../../02-guides/upgrading/codemods.mdx#remove-a-tags-from-link-components) is provided to automatically update your codebase. |
