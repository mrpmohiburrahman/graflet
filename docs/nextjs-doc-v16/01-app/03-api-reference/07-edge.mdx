...
- The **Edge Runtime** which contains a more limited [set of APIs](#reference), used in [Proxy](03-file-conventions/proxy.mdx).

## Caveats

- The Edge Runtime does not support all Node.js APIs. Some packages may not work as expected.
- The Edge Runtime does not support Incremental Static Regeneration (ISR).
- Both runtimes can support [streaming](03-file-conventions/loading.mdx) depending on your deployment adapter.
...
You can use `process.env` to access [Environment Variables](../02-guides/environment-variables.mdx) for both `next dev` and `next build`.
