...
- Warning if invalid values for [segment config options](../03-file-conventions/02-route-segment-config/index.mdx) are passed.
...
2. **Streamlined data flow between components**: With the removal of `_app` in favor of root layouts, it is now easier to visualize the data flow between components and pages. Previously, data flowing between individual `pages` and `_app` were difficult to type and could introduce confusing bugs. With [colocated data fetching](../../01-getting-started/06-fetching-data.mdx) in the App Router, this is no longer an issue.

[Data Fetching in Next.js](../../01-getting-started/06-fetching-data.mdx) now provides as close to end-to-end type safety as possible without being prescriptive about your database or content provider selection.
...
Next.js generates global helpers for App Router route types. These are available without imports and are generated during `next dev`, `next build`, or via [`next typegen`](../06-cli/next.mdx#next-typegen-options):

- [`PageProps`](../03-file-conventions/page.mdx#page-props-helper)
- [`LayoutProps`](../03-file-conventions/layout.mdx#layout-props-helper)
- [`RouteContext`](../03-file-conventions/route.mdx#route-context-helper)
...
Running `next dev`, `next build`, or [`next typegen`](../06-cli/next.mdx#next-typegen-options) regenerates this file.
...
For [`getStaticProps`](../../../02-pages/04-api-reference/03-functions/get-static-props.mdx), [`getStaticPaths`](../../../02-pages/04-api-reference/03-functions/get-static-paths.mdx), and [`getServerSideProps`](../../../02-pages/04-api-reference/03-functions/get-server-side-props.mdx), you can use the `GetStaticProps`, `GetStaticPaths`, and `GetServerSideProps` types respectively:
...
If you have a [custom `App`](../../../02-pages/03-building-your-application/01-routing/05-custom-app.mdx), you can use the built-in type `AppProps` and change file name to `./pages/_app.tsx` like so:
...
Open `next.config.ts` and enable the `ignoreBuildErrors` option in the [`typescript`](01-next-config-js/typescript.mdx) config:
...
| `v12.0.0` | [SWC](../../../03-architecture/nextjs-compiler.mdx) is now used by default to compile TypeScript and TSX for faster builds.                    |
