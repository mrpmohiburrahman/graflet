...
When leveraging [`basePath` support](basePath.mdx) with headers each `source` is automatically prefixed with the `basePath` unless you add `basePath: false` to the header:
...
When leveraging [`i18n` support](../../../02-guides/internationalization.mdx) with headers each `source` is automatically prefixed to handle the configured `locales` unless you add `locale: false` to the header. If `locale: false` is used you must prefix the `source` with a locale for it to be matched correctly.

</AppOnly>

<PagesOnly>

When leveraging [`i18n` support](../../../../02-pages/02-guides/internationalization.mdx) with headers each `source` is automatically prefixed to handle the configured `locales` unless you add `locale: false` to the header. If `locale: false` is used you must prefix the `source` with a locale for it to be matched correctly.
...
Learn more about [caching](../../../01-getting-started/08-caching.mdx) with the App Router.

</AppOnly>

<PagesOnly>

If you need to revalidate the cache of a page that has been [statically generated](../../../../02-pages/03-building-your-application/02-rendering/02-static-site-generation.mdx), you can do so by setting the `revalidate` prop in the page's [`getStaticProps`](../../../../02-pages/03-building-your-application/03-data-fetching/01-get-static-props.mdx) function.

To cache the response from an [API Route](../../../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx), you can use `res.setHeader`:
...
**This header has been superseded by CSP's `frame-ancestors` option**, which has better support in modern browsers (see [Content Security Policy](../../../02-guides/content-security-policy.mdx) for configuration details).
...
Learn more about adding a [Content Security Policy](../../../02-guides/content-security-policy.mdx) to your application.
