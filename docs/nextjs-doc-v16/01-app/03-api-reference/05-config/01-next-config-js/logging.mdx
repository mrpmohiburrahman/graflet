...
Any `fetch` requests that are restored from the [Server Components HMR cache](serverComponentsHmrCache.mdx) are not logged by default. However, this can be enabled by setting `logging.fetches.hmrRefreshes` to `true`.
