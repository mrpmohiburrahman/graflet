...
Rewrites act as a URL proxy and mask the destination path, making it appear the user hasn't changed their location on the site. In contrast, [redirects](redirects.mdx) will reroute to a new page and show the URL changes.

</AppOnly>

<PagesOnly>

Rewrites act as a URL proxy and mask the destination path, making it appear the user hasn't changed their location on the site. In contrast, [redirects](../../../../02-pages/04-api-reference/04-config/01-next-config-js/redirects.mdx) will reroute to a new page and show the URL changes.
...
1. [headers](headers.mdx) are checked/applied
2. [redirects](redirects.mdx) are checked/applied
3. [proxy](../../03-file-conventions/proxy.mdx)
4. `beforeFiles` rewrites: for each entry, if `source`, `has`, and `missing` matches the request, it's rewritten to `destination`.
5. static files from the [public directory](../../03-file-conventions/public-folder.mdx), `_next/static` files, and non-dynamic pages are checked/served
...
8. `fallback` rewrites are checked/applied, these are applied before rendering the 404 page and after dynamic routes/all static assets have been checked. If you use [fallback: true/'blocking'](../../../../02-pages/04-api-reference/03-functions/get-static-paths.mdx#fallback-true) in `getStaticPaths`, those dynamic routes take priority over the fallback `rewrites` defined in your `next.config.js`.

</AppOnly>

<PagesOnly>

1. [headers](../../../../02-pages/04-api-reference/04-config/01-next-config-js/headers.mdx) are checked/applied
2. [redirects](../../../../02-pages/04-api-reference/04-config/01-next-config-js/redirects.mdx) are checked/applied
3. `beforeFiles` rewrites: for each entry, if `source` matches the request, it's rewritten to `destination`.
4. static files from the [public directory](../../../../02-pages/04-api-reference/02-file-conventions/public-folder.mdx), `_next/static` files, and non-dynamic pages are checked/served
...
7. `fallback` rewrites are checked/applied, these are applied before rendering the 404 page and after dynamic routes/all static assets have been checked. If you use [fallback: true/'blocking'](../../../../02-pages/04-api-reference/03-functions/get-static-paths.mdx#fallback-true) in `getStaticPaths`, the fallback `rewrites` defined in your `next.config.js` will _not_ be run.
...
> **Good to know**: Static pages from [Automatic Static Optimization](../../../../02-pages/03-building-your-application/02-rendering/04-automatic-static-optimization.mdx) or [prerendering](../../../../02-pages/03-building-your-application/03-data-fetching/01-get-static-props.mdx) params from rewrites will be parsed on the client after hydration and provided in the query.
...
When leveraging [`basePath` support](basePath.mdx) with rewrites each `source` and `destination` is automatically prefixed with the `basePath` unless you add `basePath: false` to the rewrite:
