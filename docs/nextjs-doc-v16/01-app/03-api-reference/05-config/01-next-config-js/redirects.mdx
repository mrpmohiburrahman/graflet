...
When using the Pages Router, redirects are not applied to client-side routing (`Link`, `router.push`) unless [Proxy](../../03-file-conventions/proxy.mdx) is present and matches the path.
...
When leveraging [`basePath` support](basePath.mdx) with redirects each `source` and `destination` is automatically prefixed with the `basePath` unless you add `basePath: false` to the redirect:
...
For dynamic or per-request locale handling, use [dynamic route segments and proxy](../../../02-guides/internationalization.mdx), which can redirect based on the user's preferred language.
...
- Inside [API Routes](../../../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx) and [Route Handlers](../../03-file-conventions/route.mdx), you can redirect based on the incoming request.
- Inside [`getStaticProps`](../../../../02-pages/03-building-your-application/03-data-fetching/01-get-static-props.mdx) and [`getServerSideProps`](../../../../02-pages/03-building-your-application/03-data-fetching/03-get-server-side-props.mdx), you can redirect specific pages at request-time.
