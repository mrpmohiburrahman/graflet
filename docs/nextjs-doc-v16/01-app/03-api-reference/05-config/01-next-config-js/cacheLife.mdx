...
The `cacheLife` option allows you to define **custom cache profiles** when using the [`cacheLife`](../../04-functions/cacheLife.mdx) function inside components or functions, and within the scope of the [`use cache` directive](../../01-directives/use-cache.mdx).

## Usage

To define a profile, enable the [`cacheComponents` flag](cacheComponents.mdx) and add the cache profile in the `cacheLife` object in the `next.config.js` file. For example, a `blog` profile:
