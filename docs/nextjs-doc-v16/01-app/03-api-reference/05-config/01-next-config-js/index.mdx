...
The `unstable_getResponseFromNextConfig` function runs the [`headers`](headers.mdx), [`redirects`](redirects.mdx), and [`rewrites`](rewrites.mdx) functions from `next.config.js` with the provided request information and returns `NextResponse` with the results of the routing.
