...
|      <Check size={18} />      | @next/next/no-typos                                                                                                      | Prevent common typos in [Next.js's data fetching functions](../../../02-pages/03-building-your-application/03-data-fetching/index.mdx) |
...
> The `eslint-config-next/core-web-vitals` configuration is automatically included for new applications built with [Create Next App](../06-cli/create-next-app.mdx).
...
- Conflicting plugins installed separately or through another config (such as `airbnb` or `react-app`):
  ...
  - `jsx-a11y`
  - `import`
- Custom `parserOptions` different from Next.js defaults (only if you have [customized your Babel configuration](../../../02-pages/02-guides/babel.mdx))
...
| Version   | Changes                                                                                                                                                                                                             |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v16.0.0` | `next lint` and the `eslint` next.config.js option were removed in favor of the ESLint CLI. A [codemod](../../02-guides/upgrading/codemods.mdx#migrate-from-next-lint-to-eslint-cli) is available to help you migrate. |
