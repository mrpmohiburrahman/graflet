...
> - If you're coming from migrating `metadata` exports, you can use [metadata-to-viewport-export codemod](../../02-guides/upgrading/codemods.mdx#metadata-to-viewport-export) to update your changes.
...
In TypeScript, the `params` argument can be typed via [`PageProps<'/route'>`](../03-file-conventions/page.mdx#page-props-helper) or [`LayoutProps<'/route'>`](../03-file-conventions/layout.mdx#layout-props-helper) depending on where `generateViewport` is defined.
...
When [Cache Components](../../01-getting-started/08-caching.mdx) is enabled, `generateViewport` follows the same rules as other components. If viewport accesses runtime data (`cookies()`, `headers()`, `params`, `searchParams`) or performs uncached data fetching, it defers to request time.
...
> **Good to know**: Use [multiple root layouts](../03-file-conventions/layout.mdx#root-layout) to isolate fully dynamic viewport to specific routes, while still letting other routes in your application generate a static shell.

## Types

You can add type safety to your viewport object by using the `Viewport` type. If you are using the [built-in TypeScript plugin](../05-config/02-typescript.mdx) in your IDE, you do not need to manually add the type, but you can still explicitly add it if you want.
