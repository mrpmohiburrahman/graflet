...
`cookies` is an **async** function that allows you to read the HTTP incoming request cookies in [Server Components](../../01-getting-started/05-server-and-client-components.mdx), and read/write outgoing request cookies in [Server Functions](../../01-getting-started/07-mutating-data.mdx) or [Route Handlers](../03-file-conventions/route.mdx).
...
- `cookies` is a [Request-time API](../../04-glossary.mdx#request-time-apis) whose returned values cannot be known ahead of time. Using it in a layout or page will opt a route into [dynamic rendering](../../04-glossary.mdx#dynamic-rendering).
- The `.delete` method can only be called:
  - In a [Server Function](../../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-file-conventions/route.mdx).
...
- HTTP does not allow setting cookies after streaming starts, so you must use `.set` in a [Server Function](../../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-file-conventions/route.mdx).
...
After you set or delete a cookie in a Server Function, Next.js can return both the updated UI and new data in a single server roundtrip when the function is used as a [Server Action](../../01-getting-started/07-mutating-data.mdx#what-are-server-functions) (e.g., passed to a form's `action` prop). See [Caching and Revalidating](../../01-getting-started/08-caching.mdx).

The UI is not unmounted, but effects that depend on data coming from the server will re-run.

To refresh cached data too, call [`revalidatePath`](revalidatePath.mdx) or [`revalidateTag`](revalidateTag.mdx) inside the function.
...
You can use the `(await cookies()).set(name, value, options)` method in a [Server Function](../../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-file-conventions/route.mdx) to set a cookie. The [`options` object](#options) is optional.
...
| Version      | Changes                                                                                                |
| ------------ | ------------------------------------------------------------------------------------------------------ |
| `v15.0.0-RC` | `cookies` is now an async function. A [codemod](../../02-guides/upgrading/codemods.mdx#150) is available. |
