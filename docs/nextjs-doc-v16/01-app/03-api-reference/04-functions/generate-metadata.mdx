...
Otherwise the metadata resolved from `generateMetadata` [can be streamed](generate-metadata.mdx#streaming-metadata) after sending the initial UI.
...
For type completion of `params` and `searchParams`, you can type the first argument with [`PageProps<'/route'>`](../03-file-conventions/page.mdx#page-props-helper) or [`LayoutProps<'/route'>`](../03-file-conventions/layout.mdx#layout-props-helper) for pages and layouts respectively.
...
> - `fetch` requests inside `generateMetadata` are automatically [memoized](../../04-glossary.mdx#memoization) for the same data across `generateMetadata`, `generateStaticParams`, Layouts, Pages, and Server Components.
> - React [`cache` can be used](https://react.dev/reference/react/cache) if `fetch` is unavailable.
> - [File-based metadata](../03-file-conventions/01-metadata/index.mdx) has the higher priority and will override the `metadata` object and `generateMetadata` function.
...
- `props` - An object containing the parameters of the current route:
  - `params` - An object containing the [dynamic route parameters](../03-file-conventions/dynamic-routes.mdx) object from the root segment down to the segment `generateMetadata` is called from. Examples:
...
> - The [`redirect()`](redirect.mdx) and [`notFound()`](not-found.mdx) Next.js methods can also be used inside `generateMetadata`.
...
> **Good to know**:
>
> - It may be more convenient to use the [file-based Metadata API](../03-file-conventions/01-metadata/opengraph-image.mdx#image-files-jpg-png-gif) for Open Graph images. Rather than having to sync the config export with actual files, the file-based API will automatically generate the correct metadata for you.
...
> **Good to know**: We recommend using the [file-based Metadata API](../03-file-conventions/01-metadata/app-icons.mdx#image-files-ico-jpg-png) for icons where possible. Rather than having to sync the config export with actual files, the file-based API will automatically generate the correct metadata for you.
...
> **Deprecated**: The `viewport` option in `metadata` is deprecated as of Next.js 14. Please use the [`viewport` configuration](generate-viewport.mdx) instead.
...
You can add type safety to your metadata by using the `Metadata` type. If you are using the [built-in TypeScript plugin](../05-config/02-typescript.mdx) in your IDE, you do not need to manually add the type, but you can still explicitly add it if you want.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `<meta http-equiv="...">`     | Use appropriate HTTP Headers via [`redirect()`](redirect.mdx), [Proxy](../03-file-conventions/proxy.mdx#nextresponse), [Security Headers](../05-config/01-next-config-js/headers.mdx) |
| `<base>`                      | Render the tag in the layout or page itself.                                                                                                                                                                                                 |
| `<noscript>`                  | Render the tag in the layout or page itself.                                                                                                                                                                                                 |
| `<style>`                     | Learn more about [styling in Next.js](../../01-getting-started/11-css.mdx).                                                                                                                                                                        |
| `<script>`                    | Learn more about [using scripts](../../02-guides/scripts.mdx).                                                                                                                                                                                  |
...
When [Cache Components](../../01-getting-started/08-caching.mdx) is enabled, `generateMetadata` follows the same rules as other components. If metadata accesses runtime data (`cookies()`, `headers()`, `params`, `searchParams`) or performs uncached data fetching, it defers to request time.
...
| `v13.2.0` | `viewport`, `themeColor`, and `colorScheme` deprecated in favor of the [`viewport` configuration](generate-viewport.mdx). |
