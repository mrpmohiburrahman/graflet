...
It's useful when a component doesn't use [Request-time APIs](../../04-glossary.mdx#request-time-apis) like `cookies` or `headers`, but still needs to produce different output per request, such as `Math.random()` or `new Date()`.
...
- `connection` replaces [`unstable_noStore`](unstable_noStore.mdx) to better align with the future of Next.js.
