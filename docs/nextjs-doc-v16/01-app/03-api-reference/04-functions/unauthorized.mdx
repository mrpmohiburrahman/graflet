...
The `unauthorized` function throws an error that renders a Next.js 401 error page. It's useful for handling authorization errors in your application. You can customize the UI using the [`unauthorized.js` file](../03-file-conventions/unauthorized.mdx).

To start using `unauthorized`, enable the experimental [`authInterrupts`](../05-config/01-next-config-js/authInterrupts.mdx) configuration option in your `next.config.js` file:
...
`unauthorized` can be invoked in [Server Components](../../01-getting-started/05-server-and-client-components.mdx), [Server Functions](../../01-getting-started/07-mutating-data.mdx), and [Route Handlers](../03-file-conventions/route.mdx).
...
- The `unauthorized` function cannot be called in the [root layout](../03-file-conventions/layout.mdx#root-layout).
