...
An object containing the [dynamic route parameters](../03-file-conventions/dynamic-routes.mdx) object from the root segment down to the segment `generateImageMetadata` is called from.
...
A promise that resolves to an object containing the [dynamic route parameters](../03-file-conventions/dynamic-routes.mdx) from the root segment down to the segment the image is colocated in.
...
This example uses the `params` object and external data to generate multiple [Open Graph images](../03-file-conventions/01-metadata/opengraph-image.mdx) for a route segment.
