...
The `useRouter` hook allows you to programmatically change routes inside [Client Components](../../01-getting-started/05-server-and-client-components.mdx).

> **Recommendation:** Use the [`<Link>` component](../02-components/link.mdx) for navigation unless you have a specific requirement for using `useRouter`.
...
- `router.refresh()`: Refresh the current route. Making a new request to the server, re-fetching data requests, and re-rendering Server Components. The client will merge the updated React Server Component payload without losing unaffected client-side React (e.g. `useState`) or browser state (e.g. scroll position). This clears the [Client Cache](../../04-glossary.mdx#client-cache) for the current route, but does **not** invalidate the server-side cache. Use [`revalidatePath`](revalidatePath.mdx) or [`revalidateTag`](revalidateTag.mdx) to invalidate server-side cached data.
- `router.prefetch(href: string, options?: { onInvalidate?: () => void })`: [Prefetch](../../01-getting-started/04-linking-and-navigating.mdx#prefetching) the provided route for faster client-side transitions. The optional `onInvalidate` callback is called when the [prefetched data becomes stale](../../02-guides/prefetching.mdx#extending-or-ejecting-link).
...
- The `pathname` string has been removed and is replaced by [`usePathname()`](use-pathname.mdx)
- The `query` object has been removed and is replaced by [`useSearchParams()`](use-search-params.mdx)
- `router.events` has been replaced. [See below.](#router-events)

[View the full migration guide](../../02-guides/migrating/app-router-migration.mdx).
...
> **Good to know**: `<NavigationEvents>` is wrapped in a [`Suspense` boundary](../03-file-conventions/loading.mdx#examples) because[`useSearchParams()`](use-search-params.mdx) causes client-side rendering up to the closest `Suspense` boundary during [prerendering](../../04-glossary.mdx#prerendering). [Learn more](use-search-params.mdx#behavior).
