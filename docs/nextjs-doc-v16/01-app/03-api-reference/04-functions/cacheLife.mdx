...
The `cacheLife` function is used to set the cache lifetime of a function or component. It should be used alongside the [`use cache`](../01-directives/use-cache.mdx) directive, and within the scope of the function or component.

## Usage

### Basic setup

To use `cacheLife`, first enable the [`cacheComponents` flag](../05-config/01-next-config-js/cacheComponents.mdx) in your `next.config.js` file:
...
- If omitted, defaults to the `default` profile's `stale` value (5 minutes, see [`staleTimes`](../05-config/01-next-config-js/staleTimes.mdx))
...
- Similar to [Incremental Static Regeneration (ISR)](../../02-guides/incremental-static-regeneration.mdx)
...
The `stale` property controls the [Client Cache](../../04-glossary.mdx#client-cache), not the `Cache-Control` header:
...
When you call revalidation functions from a Server Action ([`revalidateTag`](revalidateTag.mdx), [`revalidatePath`](revalidatePath.mdx), [`updateTag`](updateTag.mdx), or [`refresh`](refresh.mdx)), the entire client cache is immediately cleared, bypassing the stale time.

> **Good to know**: The `stale` property in `cacheLife` differs from [`staleTimes`](../05-config/01-next-config-js/staleTimes.mdx). While `staleTimes` is a global setting affecting all routes, `cacheLife` allows per-function or per-route configuration. Updating `staleTimes.static` also updates the `stale` value of the `default` cache profile.
