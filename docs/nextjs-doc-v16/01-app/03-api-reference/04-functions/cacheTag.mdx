...
To use `cacheTag`, enable the [`cacheComponents` flag](../05-config/01-next-config-js/cacheComponents.mdx) in your `next.config.js` file:
...
You can then purge the cache on-demand from a [Server Function](../../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-file-conventions/route.mdx):

- Use [`updateTag`](updateTag.mdx) inside a Server Function for read-your-own-writes scenarios, where a user makes a change and the next read should fetch fresh data immediately.
- Use [`revalidateTag`](revalidateTag.mdx) when it is acceptable to serve stale data while revalidation happens in the background, or when revalidating from a route handler.
...
Using [`revalidateTag`](revalidateTag.mdx), you can invalidate the cache for a specific tag when needed:
