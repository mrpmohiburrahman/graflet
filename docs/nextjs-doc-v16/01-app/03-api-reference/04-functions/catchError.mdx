...
The `unstable_catchError` function creates a component that wraps its children in an error boundary. It provides a programmatic alternative to the [`error.js`](../03-file-conventions/error.mdx) file convention, enabling component-level error recovery anywhere in your component tree.

Compared to a custom React error boundary, `unstable_catchError` is designed to work with Next.js out of the box:

- **Built-in error recovery** — [`unstable_retry()`](../03-file-conventions/error.mdx#unstable_retry) re-renders the page inside a [Transition](https://react.dev/reference/react/startTransition), preserving Client Components state outside of the error boundary.
...
`unstable_catchError` can be called from [Client Components](../../01-getting-started/05-server-and-client-components.mdx).
...
| `reset`          | `() => void`                                                                                | Resets the error state and re-renders without re-fetching. Use [`unstable_retry()`](../03-file-conventions/error.mdx#unstable_retry) in most cases. |
...
> - You don't need to wrap `error.js` default exports with `unstable_catchError`. The [`error.js`](../03-file-conventions/error.mdx) file convention already renders inside a built-in error boundary provided by Next.js.
