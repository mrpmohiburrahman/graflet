...
**In version 15, we recommend using [`connection`](connection.mdx) instead of `unstable_noStore`.**
...
- Using `unstable_noStore` inside [`unstable_cache`](unstable_cache.mdx) will not opt out of static generation. Instead, it will defer to the cache configuration to determine whether to cache the result or not.
