...
`headers` is an **async** function that allows you to **read** the HTTP incoming request headers from a [Server Component](../../01-getting-started/05-server-and-client-components.mdx).
...
- `headers` is a [Request-time API](../../04-glossary.mdx#request-time-apis) whose returned values cannot be known ahead of time. Using it in will opt a route into **[dynamic rendering](../../04-glossary.mdx#dynamic-rendering)**.
...
| Version      | Changes                                                                                                |
| ------------ | ------------------------------------------------------------------------------------------------------ |
| `v15.0.0-RC` | `headers` is now an async function. A [codemod](../../02-guides/upgrading/codemods.mdx#150) is available. |
