...
It can be used in [Server Components](../../01-getting-started/05-server-and-client-components.mdx) (including [`generateMetadata`](generate-metadata.mdx)), [Server Functions](../../01-getting-started/07-mutating-data.mdx), [Route Handlers](../03-file-conventions/route.mdx), and [Proxy](../03-file-conventions/proxy.mdx).
...
> **Good to know:** `after` is not a [Request-time API](../../04-glossary.mdx#request-time-apis) and calling it does not cause a route to become dynamic. If it's used within a static page, the callback will execute at build time, or whenever a page is revalidated.
...
`after` will run for the platform's default or configured max duration of your route. If your platform supports it, you can configure the timeout limit using the [`maxDuration`](../03-file-conventions/02-route-segment-config/maxDuration.mdx) route segment config.
...
Whether you can use request APIs like [`cookies`](cookies.mdx) and [`headers`](headers.mdx) inside `after` depends on where `after` is called from.

#### In Route Handlers and Server Functions

You can call `cookies` and `headers` directly inside the `after` callback when used in [Route Handlers](../03-file-conventions/route.mdx) and [Server Functions](../../01-getting-started/07-mutating-data.mdx). This is useful for logging activity after a mutation or API request. For example:
...
[Server Components](../../01-getting-started/05-server-and-client-components.mdx) (including pages, layouts, and `generateMetadata`) **cannot** use `cookies`, `headers`, or other Request-time APIs inside `after`. This is because Next.js needs to know which part of the component tree accesses request data to support [Partial Prerendering](../../04-glossary.mdx#partial-prerendering-ppr) and [Cache Components](../../01-getting-started/08-caching.mdx), but `after` runs after React's rendering lifecycle.
...
When using [Cache Components](../../01-getting-started/08-caching.mdx), components that access request data like `cookies` or `headers` must be wrapped in [`<Suspense>`](https://react.dev/reference/react/Suspense) so the rest of the page can be prerendered into a static shell.
...
Learn how to [configure `after`](../../02-guides/self-hosting.mdx#after) when self-hosting Next.js.
