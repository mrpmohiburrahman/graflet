...
The `redirect` function allows you to redirect the user to another URL. `redirect` can be used while rendering in [Server and Client Components](../../01-getting-started/05-server-and-client-components.mdx), [Route Handlers](../03-file-conventions/route.mdx), and [Server Functions](../../01-getting-started/07-mutating-data.mdx).

When used in a [streaming context](../../01-getting-started/04-linking-and-navigating.mdx#streaming), this will insert a meta tag to emit the redirect on the client side. When used in a server action, it will serve a 303 HTTP redirect response to the caller. Otherwise, it will serve a 307 HTTP redirect response to the caller.

If a resource doesn't exist, you can use the [`notFound` function](not-found.mdx) instead.
...
By default, `redirect` will use `push` (adding a new entry to the browser history stack) in [Server Actions](../../01-getting-started/07-mutating-data.mdx) and `replace` (replacing the current URL in the browser history stack) everywhere else. You can override this behavior by specifying the `type` parameter.
...
- If you prefer to return a 308 (Permanent) HTTP redirect instead of 307 (Temporary), you can use the [`permanentRedirect` function](permanentRedirect.mdx) instead.
- `redirect` throws an error so it should be called **outside** the `try` block when using `try/catch` statements.
- `redirect` can be called in Client Components during the rendering process but not in event handlers. You can use the [`useRouter` hook](use-router.mdx) instead.
- `redirect` also accepts absolute URLs and can be used to redirect to external links.
- If you'd like to redirect before the render process, use [`next.config.js`](../../02-guides/redirecting.mdx#redirects-in-nextconfigjs) or [Proxy](../../02-guides/redirecting.mdx#nextresponseredirect-in-proxy).
...
`redirect` can be used in a Client Component through a Server Action. If you need to use an event handler to redirect the user, you can use the [`useRouter`](use-router.mdx) hook.
