...
You should follow a **Zero Trust** model when adopting Server Components in an existing project. You can continue calling your existing API endpoints such as REST or GraphQL from Server Components using [`fetch`](../03-api-reference/04-functions/fetch.mdx), just as you would in Client Components.
...
You can enable usage in your Next.js app with the [`experimental.taint`](../03-api-reference/05-config/01-next-config-js/taint.mdx) option in `next.config.js`:
...
> **Good to know:**
>
> - By default, environment variables are only available on the Server. Next.js exposes any environment variable prefixed with `NEXT_PUBLIC_` to the client. [Learn more](environment-variables.mdx).
...
Read more about `server-only` in the [preventing environment poisoning](../01-getting-started/05-server-and-client-components.mdx#preventing-environment-poisoning) section.
...
Learn more about [Authentication](authentication.mdx) in Next.js.
...
For expensive operations (sending emails, writing to a database), consider adding rate limiting to prevent abuse. See the [Rate limiting](backend-for-frontend.mdx#rate-limiting) example in the Backend for Frontend guide.
...
This is an advanced use case where consistent encryption behavior across multiple deployments is critical for your application. Follow standard security practices such as key rotation and signing. See the [Self-Hosting guide](self-hosting.mdx#server-functions-encryption-key) for deployment-specific considerations.
