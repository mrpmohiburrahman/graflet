...
- [Route Handlers](../03-api-reference/03-file-conventions/route.mdx)
- [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx)
- In Pages Router, [API Routes](../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx)
...
To restrict access, implement authentication and authorization. See [Authentication](authentication.mdx).
...
- [`sitemap.xml`](../03-api-reference/03-file-conventions/01-metadata/sitemap.mdx)
- [`opengraph-image.jpg`, `twitter-image`](../03-api-reference/03-file-conventions/01-metadata/opengraph-image.mdx)
- [favicon, app icon, and apple-icon](../03-api-reference/03-file-conventions/01-metadata/app-icons.mdx)
- [`manifest.json`](../03-api-reference/03-file-conventions/01-metadata/manifest.mdx)
- [`robots.txt`](../03-api-reference/03-file-conventions/01-metadata/robots.mdx)
...
You can use [rewrites](../03-api-reference/05-config/01-next-config-js/rewrites.mdx) with header matching to serve different content types from the same URL based on the request's `Accept` header. This is known as [content negotiation](https://developer.mozilla.org/en-US/docs/Web/HTTP/Content_negotiation).
...
> **Good to know:**
>
> - The `/docs/md/...` route is still directly accessible without the rewrite. If you want to restrict it to only serve via the rewrite, use [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx) to block direct requests that don't include the expected `Accept` header.
> - For more advanced negotiation logic, you can use [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx) instead of rewrites for more flexibility.
...
- [`rewrites`](../03-api-reference/05-config/01-next-config-js/rewrites.mdx) in `next.config.js`.
...
`NextRequest` includes the [`nextUrl`](../03-api-reference/04-functions/next-request.mdx#nexturl) property, which exposes parsed values from the incoming request, for example, it makes it easier to access request pathname and search params.
...
Learn more about [`NextRequest`](../03-api-reference/04-functions/next-request.mdx) and [`NextResponse`](../03-api-reference/04-functions/next-response.mdx).
...
Learn more about redirects in [`redirect`](../03-api-reference/04-functions/redirect.mdx) and [`permanentRedirect`](../03-api-reference/04-functions/permanentRedirect.mdx)

## Proxy

Only one `proxy` file is allowed per project. Use `config.matcher` to target specific paths. Learn more about [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx).
...
Learn more in [NextResponse headers in Proxy](../03-api-reference/04-functions/next-response.mdx#next).
...
- [CORS](../03-api-reference/03-file-conventions/route.mdx#cors)
...
See more examples on using [Router Handlers](../03-api-reference/03-file-conventions/route.mdx#examples) and the [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx#examples) API references.

These examples include, working with [Cookies](../03-api-reference/03-file-conventions/route.mdx#cookies), [Headers](../03-api-reference/03-file-conventions/route.mdx#headers), [Streaming](../03-api-reference/03-file-conventions/route.mdx#streaming), Proxy [negative matching](../03-api-reference/03-file-conventions/proxy.mdx#negative-matching), and other useful code snippets.
...
In `export mode`, only `GET` Route Handlers are supported, in combination with the [`dynamic`](caching-without-cache-components.mdx#dynamic) route segment config, set to `'force-static'`.
