...
With [Cache Components](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) enabled, wrapping uncached data in `<Suspense>` boundaries produces instant navigations — but only if the boundaries are in the right place. A misplaced boundary can silently block client-side navigations, especially where the entry point varies by shared layout. **Always export `unstable_instant` from routes that should navigate instantly** — it validates the caching structure at dev time and build time, catching issues before they reach users.
...
The [`unstable_instant`](../03-api-reference/03-file-conventions/02-route-segment-config/instant.mdx) export on line 1 tells Next.js to validate that this page produces an instant [static shell](../04-glossary.mdx#static-shell) at every possible entry point. Validation runs during development and at build time. If a component would block navigation, the error overlay tells you exactly which one and suggests a fix.
...
- [`instant` API reference](../03-api-reference/03-file-conventions/02-route-segment-config/instant.mdx) for all configuration options, including runtime prefetching and incremental adoption with `instant = false`
- [Caching](../01-getting-started/08-caching.mdx) for background on `use cache`, Suspense, and Partial Prerendering
- [Revalidating](../01-getting-started/09-revalidating.mdx) for how to expire cached data with `cacheLife` and `updateTag`
