...
[Proxy](../03-api-reference/03-file-conventions/proxy.mdx) enables you to add headers and generate nonces before the page renders.

Every time a page is viewed, a fresh nonce should be generated. This means that you **must use [dynamic rendering](../04-glossary.mdx#dynamic-rendering) to add nonces**.
...
By default, Proxy runs on all requests. You can filter Proxy to run on specific paths using a [`matcher`](../03-api-reference/03-file-conventions/proxy.mdx#matcher).
...
You can read the nonce from a [Server Component](../01-getting-started/05-server-and-client-components.mdx) using [`headers`](../03-api-reference/04-functions/headers.mdx):
...
For applications that do not require nonces, you can set the CSP header directly in your [`next.config.js`](../03-api-reference/05-config/01-next-config-js/index.mdx) file:
