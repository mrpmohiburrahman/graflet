...
A zone is a normal Next.js application where you also configure an [assetPrefix](../03-api-reference/05-config/01-next-config-js/assetPrefix.mdx) to avoid conflicts with pages and static files in other zones.
...
Routing requests through [`rewrites`](../03-api-reference/05-config/01-next-config-js/rewrites.mdx) is recommended to minimize latency overhead for the requests, but proxy can also be used when there is a need for a dynamic decision when routing. For example, if you are using a feature flag to decide where a path should be routed such as during a migration, you can use proxy.
...
Links to paths in a different zone should use an `a` tag instead of the Next.js [`<Link>`](../03-api-reference/02-components/link.mdx) component. This is because Next.js will try to prefetch and soft navigate to any relative path in `<Link>` component, which will not work across zones.
...
When using [Server Actions](../01-getting-started/07-mutating-data.mdx) with Multi-Zones, you must explicitly allow the user-facing origin since your user facing domain may serve multiple applications. In your `next.config.js` file, add the following lines:
...
See [`serverActions.allowedOrigins`](../03-api-reference/05-config/01-next-config-js/serverActions.mdx#allowedorigins) for more information.
