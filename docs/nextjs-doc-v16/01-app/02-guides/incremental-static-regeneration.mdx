...
6. If `/blog/26` is requested, and it exists, the page will be generated on-demand. This behavior can be changed by using a different [dynamicParams](../03-api-reference/03-file-conventions/02-route-segment-config/dynamicParams.mdx) value. However, if the post does not exist, then 404 is returned.
...
4. The cache is invalidated and a new version of the page begins generating in the background
5. Once generated successfully, the next request will return the updated page and cache it for subsequent requests
6. If `/blog/26` is requested, and it exists, the page will be generated on-demand. This behavior can be changed by using a different [fallback](../../02-pages/04-api-reference/03-functions/get-static-paths.mdx#fallback-false) value. However, if the post does not exist, then 404 is returned.
...
- [`revalidate`](caching-without-cache-components.mdx#route-segment-config-revalidate)
- [`dynamicParams`](../03-api-reference/03-file-conventions/02-route-segment-config/dynamicParams.mdx)

### Functions

- [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx)
- [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx)
...
- [`res.revalidate`](../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx#response-helpers)
...
We recommend setting a high revalidation time. For instance, 1 hour instead of 1 second. If you need more precision, consider using on-demand revalidation. If you need real-time data, consider switching to [dynamic rendering](../04-glossary.mdx#dynamic-rendering).
...
You can then use `revalidateTag` in a [Server Actions](../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-api-reference/03-file-conventions/route.mdx):
...
If an error is thrown while attempting to revalidate data, the last successfully generated data will continue to be served from the cache. On the next subsequent request, Next.js will retry revalidating the data. [Learn more about error handling](../01-getting-started/10-error-handling.mdx).
...
You can configure the Next.js cache location if you want to persist cached pages and data to durable storage, or share the cache across multiple containers or instances of your Next.js application. [Learn more](self-hosting.mdx#caching-and-isr).

## Troubleshooting

### Debugging cached data in local development

If you are using the `fetch` API, you can add additional logging to understand which requests are cached or uncached. [Learn more about the `logging` option](../03-api-reference/05-config/01-next-config-js/logging.mdx).
...
- ISR is not supported when creating a [Static Export](static-exports.mdx).
- If you have multiple `fetch` requests in a prerendered route, and each has a different `revalidate` frequency, the lowest time will be used for ISR. However, those revalidate frequencies will still be respected by the [cache](../01-getting-started/08-caching.mdx).
...
- When running multiple instances, the default file-system cache is per-instance. On-demand revalidation only invalidates the instance that receives the call. Use a shared [custom cache handler](../03-api-reference/05-config/01-next-config-js/incrementalCacheHandlerPath.mdx) to coordinate across instances. See [How Revalidation Works](how-revalidation-works.mdx) for the full architecture.
...
Learn how to [configure ISR](self-hosting.mdx#caching-and-isr) when self-hosting Next.js.
