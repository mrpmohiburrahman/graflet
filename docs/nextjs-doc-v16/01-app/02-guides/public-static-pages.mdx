...
Since data is shared, these kind of pages can be [prerendered](../04-glossary.mdx#prerendering) ahead of time and reused. This leads to faster page loads and lower server costs.
...
We'll start with a static header, add a product list with async external data, and learn how to render it without blocking the response. Finally, we'll add a user-specific promotion banner without switching the entire page to [dynamic rendering](../04-glossary.mdx#dynamic-rendering).
...
> **Good to know**: If you need to display locale-aware dates or times without a visible flash, see [Preventing flash before hydration](preventing-flash-before-hydration.mdx).

Since its output never changes and can be determined ahead of time, this kind of component is called a **static** component. With no reason to wait for a request, Next.js can safely **prerender** the page at [build time](../04-glossary.mdx#build-time).

We can confirm this by running [`next build`](../03-api-reference/06-cli/next.mdx#next-build-options).
...
- [**Cache**](../04-glossary.mdx#cache-components) the component, so it becomes **stable** and can be prerendered with the rest of the page.
- [**Stream**](../04-glossary.mdx#streaming) the component, so it becomes **non-blocking** and the rest of the page doesn't have to wait for it.

In our case, the product catalog is shared across all users, so caching is the right choice.

### Cache components

We can mark a function as cacheable using the [`'use cache'`](../03-api-reference/01-directives/use-cache.mdx) directive.
...
This turns it into a [cache component](../04-glossary.mdx#cache-components). The first time it runs, whatever we return will be cached and reused.
...
Next.js supports streaming by default. We can use a [Suspense boundary](../04-glossary.mdx#suspense-boundary) to tell the framework where to slice the streamed response into _chunks_, and what fallback UI to show while content loads.
...
With this change, Next.js can separate prerenderable work from request-time work and the route becomes [partially prerendered](../04-glossary.mdx#partial-prerendering-ppr).
...
At [**build time**](../04-glossary.mdx#build-time), most of the page, including the header, product list and promotion fallback, is rendered, cached and pushed to a content delivery network.

At [**request time**](../04-glossary.mdx#dynamic-rendering), the prerendered part is served instantly from a CDN node close to the user.
