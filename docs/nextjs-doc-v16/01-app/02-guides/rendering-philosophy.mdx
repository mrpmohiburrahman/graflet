...
This is what Partial Prerendering, [Cache Components](../01-getting-started/08-caching.mdx) (`use cache`), and [on-demand revalidation](../03-api-reference/04-functions/revalidateTag.mdx) enable. They are not incremental features: they represent a rendering model that treats static and dynamic as a spectrum rather than a binary choice.
...
- **Incremental caching.** Developers can add caching and revalidation incrementally, without deciding upfront at build time whether a route is static or dynamic. Any page can be revalidated on demand, and any function can be cached with [`use cache`](../03-api-reference/01-directives/use-cache.mdx).
- **Granular caching.** Cache a function with [`use cache`](../03-api-reference/01-directives/use-cache.mdx), not a route. Revalidate a [tag](../03-api-reference/04-functions/revalidateTag.mdx), not a deployment. This means an expensive database query can be cached independently of the rest of the page.
...
- **Streaming** is required because static and dynamic content are served in a single response. The server sends initial content first, then streams dynamic portions as they resolve. See [Streaming](streaming.mdx) for details.
- **Cache coordination** is required when running multiple instances because any cached content can be invalidated on demand via [`revalidateTag()`](../03-api-reference/04-functions/revalidateTag.mdx) or [`revalidatePath()`](../03-api-reference/04-functions/revalidatePath.mdx). See [How Revalidation Works](how-revalidation-works.mdx) for the architecture.
- **Cache consistency** matters because revalidation regenerates both the HTML response and the RSC payload (the serialized React Server Components data used for client-side navigation). If these get out of sync, users may see inconsistent data during navigation. See [How Revalidation Works](how-revalidation-works.mdx) for consistency requirements.
- **PPR shell delivery at CDN latency** can require additional platform integration to store the static shell separately and resume dynamic rendering correctly. See [PPR Platform Guide](ppr-platform-guide.mdx) for implementation details.
...
A platform that achieves functional fidelity is a fully supported deployment target for Next.js. Performance fidelity is how platforms differentiate. See [Deploying to Platforms](deploying-to-platforms.mdx) for the full feature compatibility matrix.

## CDN Feature Compatibility

Many CDNs have useful primitives for deeper Next.js integration (edge compute, key-value storage, blob storage), but end-to-end PPR resume support is still emerging and may require bespoke platform work. Most community adapters today deploy Next.js as a Node.js server without leveraging these CDN-specific primitives. See the [Deploying](../01-getting-started/17-deploying.mdx#adapters) page for the current list of adapters.

See [Deploying to Platforms](deploying-to-platforms.mdx#cdn-infrastructure-compatibility) for the full CDN compatibility table and [CDN Caching](cdn-caching.mdx) for caching behavior details.
