...
With built-in support for [streaming through React Suspense](../../01-getting-started/04-linking-and-navigating.mdx#streaming), you can be more intentional about which parts of your UI you want to load first and in what order without introducing network waterfalls.
...
[Next.js Proxy](../../03-api-reference/03-file-conventions/proxy.mdx) allows you to run code on the server before a request is completed. This is especially useful to avoid having a flash of unauthenticated content when the user visits an authenticated-only page by redirecting the user to a login page. The proxy is also useful for experimentation and [internationalization](../internationalization.mdx).

### Built-in Optimizations

[Images](../../03-api-reference/02-components/image.mdx), [fonts](../../03-api-reference/02-components/font.mdx), and [third-party scripts](../scripts.mdx) often have significant impact on an application's performance. Next.js comes with built-in components that automatically optimize those for you.
...
[Next.js configuration options](../../03-api-reference/05-config/01-next-config-js/index.mdx).
...
[Next.js docs](../../03-api-reference/05-config/02-typescript.mdx#ide-plugin).

### Step 4: Create the Root Layout

A Next.js [App Router](../../index.mdx) application must include a
[root layout](../../03-api-reference/03-file-conventions/layout.mdx#root-layout)
file, which is a [React Server Component](../../01-getting-started/05-server-and-client-components.mdx)
...
5. Any [metadata files](../../01-getting-started/14-metadata-and-og-images.mdx#file-based-metadata)
   ...
   moving
   [all supported files](../../01-getting-started/14-metadata-and-og-images.mdx#file-based-metadata)
...
6. Finally, Next.js can manage your last `<head>` tags with the
   [Metadata API](../../01-getting-started/14-metadata-and-og-images.mdx). Move your final metadata
   info into an exported
   [`metadata` object](../../03-api-reference/04-functions/generate-metadata.mdx#metadata-object):
...
([Metadata API](../../01-getting-started/14-metadata-and-og-images.mdx)). This approach enables you
...
This file is a [Server Component](../../01-getting-started/05-server-and-client-components.mdx). When you run `next build`, the file is prerendered into a static asset. It does _not_ require any dynamic code.

This file imports our global CSS and tells [`generateStaticParams`](../../03-api-reference/04-functions/generate-static-params.mdx) we are only going to generate one route, the index route at `/`.
...
This file is a [Client Component](../../01-getting-started/05-server-and-client-components.mdx), defined by the `'use client'`
directive. Client Components are still [prerendered to HTML](../../01-getting-started/05-server-and-client-components.mdx#how-do-server-and-client-components-work-in-nextjs) on the server before being sent to the client.
...
Next.js [`<Image>` component](../../03-api-reference/02-components/image.mdx), or you can use the object's
`src` property with your existing `<img>` tag.

The `<Image>` component has the added benefits of
[automatic image optimization](../../03-api-reference/02-components/image.mdx). The `<Image>`
...
issues. You can then optionally later migrate to the `<Image>` component to take advantage of optimizing images by [configuring a loader](../../03-api-reference/02-components/image.mdx#loader), or moving to the default Next.js server which has automatic image optimization.
...
2. **Set [`basePath`](../../03-api-reference/05-config/01-next-config-js/basePath.mdx) to `process.env.NEXT_PUBLIC_BASE_PATH` in your `next.config.mjs` file:**
...
- [Update your ESLint configuration to support Next.js rules](../../03-api-reference/05-config/03-eslint.mdx)
