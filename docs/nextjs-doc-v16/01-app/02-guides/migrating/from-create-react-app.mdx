...
Create React App uses purely client-side rendering. Client-side only applications, also known as [single-page applications (SPAs)](../single-page-applications.mdx), often experience slow initial page loading time. This happens due to a couple of reasons:
...
With built-in support for [streaming through React Suspense](../../01-getting-started/04-linking-and-navigating.mdx#streaming), you can define which parts of your UI load first and in what order, without creating network waterfalls.
...
[Next.js Proxy](../../03-api-reference/03-file-conventions/proxy.mdx) allows you to run code on the server before a request is completed. For instance, you can avoid a flash of unauthenticated content by redirecting a user to a login page in the proxy for authenticated-only pages. You can also use it for features like A/B testing, experimentation, and [internationalization](../internationalization.mdx).

### Built-in Optimizations

[Images](../../03-api-reference/02-components/image.mdx), [fonts](../../03-api-reference/02-components/font.mdx), and [third-party scripts](../scripts.mdx) often have a large impact on an application’s performance. Next.js includes specialized components and APIs that automatically optimize them for you.

## Migration Steps

Our goal is to get a working Next.js application as quickly as possible so that you can then adopt Next.js features incrementally. To begin with, we’ll treat your application as a purely client-side application ([SPA](../single-page-applications.mdx)) without immediately replacing your existing router. This reduces complexity and merge conflicts.
...
Create a `next.config.ts` at the root of your project (same level as your `package.json`). This file holds your [Next.js configuration options](../../03-api-reference/05-config/01-next-config-js/index.mdx).
...
A Next.js [App Router](../../index.mdx) application must include a [root layout](../../03-api-reference/03-file-conventions/layout.mdx#root-layout) file, which is a [React Server Component](../../01-getting-started/05-server-and-client-components.mdx) that will wrap all your pages.
...
> **Good to know**: Next.js ignores CRA’s `public/manifest.json`, additional iconography, and [testing configuration](../testing/index.mdx) by default. If you need these, Next.js has support with its [Metadata API](../../01-getting-started/14-metadata-and-og-images.mdx) and [Testing](../testing/index.mdx) setup.
...
Any [metadata files](../../01-getting-started/14-metadata-and-og-images.mdx#file-based-metadata) such as `favicon.ico`, `icon.png`, `robots.txt` are automatically added to the application `<head>` tag as long as you have them placed into the top level of the `app` directory. After moving [all supported files](../../01-getting-started/14-metadata-and-og-images.mdx#file-based-metadata) into the `app` directory you can safely delete their `<link>` tags:
...
Finally, Next.js can manage your last `<head>` tags with the [Metadata API](../../01-getting-started/14-metadata-and-og-images.mdx). Move your final metadata info into an exported [`metadata` object](../../03-api-reference/04-functions/generate-metadata.mdx#metadata-object):
...
With the above changes, you shifted from declaring everything in your `index.html` to using Next.js' convention-based approach built into the framework ([Metadata API](../../01-getting-started/14-metadata-and-og-images.mdx)). This approach enables you to more easily improve your SEO and web shareability of your pages.

### Step 5: Styles

Like CRA, Next.js supports [CSS Modules](../../01-getting-started/11-css.mdx#css-modules) out of the box. It also supports [global CSS imports](../../01-getting-started/11-css.mdx#global-css).
...
If you're using Tailwind CSS, see our [installation docs](../../01-getting-started/11-css.mdx#tailwind-css).
...
Since we want to keep the app as an SPA for now and intercept **all** routes, we’ll use an [optional catch-all route](../../03-api-reference/03-file-conventions/dynamic-routes.mdx#optional-catch-all-segments).
...
This tells Next.js to generate a single route for the empty slug (`/`), effectively mapping **all** routes to the same page. This page is a [Server Component](../../01-getting-started/05-server-and-client-components.mdx), prerendered into static HTML.

### Step 7: Add a Client-Only Entrypoint

Next, we’ll embed your CRA’s root App component inside a [Client Component](../../01-getting-started/05-server-and-client-components.mdx) so that all logic remains client-side. If this is your first time using Next.js, it's worth knowing that clients components (by default) are still prerendered on the server. You can think about them as having the additional capability of running client-side JavaScript.
...
With Next.js, static image imports return an object. The object can then be used directly with the Next.js [`<Image>` component](../../03-api-reference/02-components/image.mdx), or you can use the object's `src` property with your existing `<img>` tag.

The `<Image>` component has the added benefits of [automatic image optimization](../../03-api-reference/02-components/image.mdx). The `<Image>` component automatically sets the `width` and `height` attributes of the resulting `<img>` based on the image's dimensions. This prevents layout shifts when the image loads. However, this can cause issues if your app contains images with only one of their dimensions being styled without the other styled to `auto`. When not styled to `auto`, the dimension will default to the `<img>` dimension attribute's value, which can cause the image to appear distorted.

Keeping the `<img>` tag will reduce the amount of changes in your application and prevent the above issues. You can then optionally later migrate to the `<Image>` component to take advantage of optimizing images by [configuring a loader](../../03-api-reference/02-components/image.mdx#loader), or moving to the default Next.js server which has automatic image optimization.
...
If you used the `homepage` field in your CRA `package.json` to serve the app under a specific subpath, you can replicate that in Next.js using the [`basePath` configuration](../../03-api-reference/05-config/01-next-config-js/basePath.mdx) in `next.config.ts`:
...
If your CRA app used the `proxy` field in `package.json` to forward requests to a backend server, you can replicate this with [Next.js rewrites](../../03-api-reference/05-config/01-next-config-js/rewrites.mdx) in `next.config.ts`:
...
Create React App uses webpack for bundling. Next.js now defaults to [Turbopack](../../03-api-reference/05-config/01-next-config-js/turbopack.mdx) for faster local development:
...
You can still provide a [custom webpack configuration](../../03-api-reference/05-config/01-next-config-js/webpack.mdx) if you need to migrate advanced webpack settings from CRA.
...
- **Migrate from React Router** to the [Next.js App Router](../../index.mdx) for:
  ...
  - [Streaming server rendering](../../03-api-reference/03-file-conventions/loading.mdx)
  - [React Server Components](../../01-getting-started/05-server-and-client-components.mdx)
- **Optimize images** with the [`<Image>` component](../../03-api-reference/02-components/image.mdx)
- **Optimize fonts** with [`next/font`](../../03-api-reference/02-components/font.mdx)
- **Optimize third-party scripts** with the [`<Script>` component](../scripts.mdx)
- **Enable ESLint** with Next.js [recommended rules](../../03-api-reference/05-config/03-eslint.mdx#setup-eslint)
