...
When navigating to the new page, there's no full page reload or browser loading spinner. Instead, Next.js performs a [client-side transition](../01-getting-started/04-linking-and-navigating.mdx#client-side-transitions), making the page navigation feel instant.

## Prefetching static vs. dynamic routes

|                                                         | **Static page** | **Dynamic page**                                                                |
| ------------------------------------------------------- | --------------- | ------------------------------------------------------------------------------- |
| **Prefetched**                                          | Yes, full route | No, unless [`loading.js`](../03-api-reference/03-file-conventions/loading.mdx)     |
| [**Client Cache TTL**](../04-glossary.mdx#client-cache) | 5 min (default) | Off, unless [enabled](../03-api-reference/05-config/01-next-config-js/staleTimes.mdx) |
| **Server roundtrip on click**                           | No              | Yes, streamed after [shell](../01-getting-started/08-caching.mdx)                  |
...
| ----------------- | -------------------------------- | ------------------------------------------------------------------------------------------------- |
| No `loading.js`   | Entire page                      | 5 min ([`staleTimes.static`](../03-api-reference/05-config/01-next-config-js/staleTimes.mdx))           |
| With `loading.js` | Layout to first loading boundary | Off by default ([`staleTimes.dynamic`](../03-api-reference/05-config/01-next-config-js/staleTimes.mdx)) |
...
Alternatively, you can use [`useRouter`](../03-api-reference/04-functions/use-router.mdx) to recreate some of the native `<Link>` behavior. However, be aware this opts you into maintaining prefetching and cache invalidation.
...
[`onInvalidate`](../03-api-reference/04-functions/use-router.mdx#userouter) is invoked when Next.js suspects cached data is stale, allowing you to refresh the prefetch.
...
When PPR is enabled, a page is divided into a [static shell](streaming.mdx#the-static-shell) and a streamed dynamic section:
