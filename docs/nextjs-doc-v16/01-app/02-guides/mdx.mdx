...
> - Learn more about the [`mdx-components.tsx` file convention](../03-api-reference/03-file-conventions/mdx-components.mdx).
...
In App Router apps, that includes being able to use [metadata](../01-getting-started/14-metadata-and-og-images.mdx).
...
[`generateStaticParams`](../03-api-reference/04-functions/generate-static-params.mdx) can be used to prerender the provided routes. By marking `dynamicParams` as `false`, accessing a route not defined in `generateStaticParams` will 404.
...
> **Good to know**: Ensure you specify the `.mdx` file extension in your import. While it is not required to use [module path aliases](../01-getting-started/01-installation.mdx#set-up-absolute-imports-and-module-path-aliases) (e.g., `@/content`), it does simplify your import path.
...
To share a layout across MDX pages, you can use the [built-in layouts support](../03-api-reference/03-file-conventions/layout.mdx) with the App Router.
...
To use plugins with [Turbopack](../03-api-reference/08-turbopack.mdx), upgrade to the latest `@next/mdx` and specify plugin names using a string:
...
> **Good to know**:
>
> remark and rehype plugins without serializable options cannot be used yet with [Turbopack](../03-api-reference/08-turbopack.mdx), because JavaScript functions can't be passed to Rust.
