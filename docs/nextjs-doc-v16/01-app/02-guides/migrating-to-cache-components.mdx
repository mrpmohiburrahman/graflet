...
When [Cache Components](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) is enabled, route segment configs like `dynamic`, `revalidate`, and `fetchCache` are replaced by [`use cache`](../03-api-reference/01-directives/use-cache.mdx) and [`cacheLife`](../03-api-reference/04-functions/cacheLife.mdx).

Start by removing the route segment configs (`dynamic`, `revalidate`, `fetchCache`). With Cache Components enabled, Next.js surfaces uncached dynamic data as errors in development, naming the code to fix, most often uncached data to cache with [`use cache`](../03-api-reference/01-directives/use-cache.mdx) or runtime data to wrap in [`<Suspense>`](https://react.dev/reference/react/Suspense).
...
Cache Components requires Next.js 16. If you're on Next.js 15 or earlier, upgrade first by following the [version 16 upgrade guide](upgrading/version-16.mdx). Coming from an older version, work through the [upgrade guides](upgrading/index.mdx) to reach 16 before continuing.

Then enable the [`cacheComponents`](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) flag in `next.config.ts`:
...
> **Good to know:** If you were using `experimental.dynamicIO` or `experimental.useCache`, `cacheComponents` replaces them. See the [version 16 upgrade guide](upgrading/version-16.mdx#experimentaldynamicio-and-experimentalusecache).
...
For uncached data access, add [`use cache`](../03-api-reference/01-directives/use-cache.mdx) as close to the data access as possible with a long [`cacheLife`](../03-api-reference/04-functions/cacheLife.mdx) like `'max'` to maintain cached behavior. If needed, add it at the top of the page or layout.
...
With Cache Components, wrap the fetch in a [`use cache`](../03-api-reference/01-directives/use-cache.mdx) function. Fetches inside that scope are cached automatically, and `revalidate` and `tags` become [`cacheLife`](../03-api-reference/04-functions/cacheLife.mdx) and [`cacheTag`](../03-api-reference/04-functions/cacheTag.mdx).
...
`use cache` defaults to in-memory storage, so its entries are discarded when the serverless instance is destroyed and are scoped to a single deployment. Use [`use cache: remote`](../03-api-reference/01-directives/use-cache-remote.mdx) or a [cache handler](../03-api-reference/05-config/01-next-config-js/cacheHandlers.mdx) for storage that survives instance teardown. Even with durable storage, expect cached values to recompute after a new deployment.

## `unstable_cache`

**Replace with `use cache`.**

`unstable_cache` is replaced by the [`use cache`](../03-api-reference/01-directives/use-cache.mdx) directive.

Turn the wrapped function into a function with the `'use cache'` directive. The cache key is derived automatically from the arguments, so the key-parts array is no longer needed, and the `options` object maps to [`cacheLife`](../03-api-reference/04-functions/cacheLife.mdx) and [`cacheTag`](../03-api-reference/04-functions/cacheTag.mdx).
...
On-demand invalidation still works by tagging cached data and expiring it after an event. Tag data with [`cacheTag`](../03-api-reference/04-functions/cacheTag.mdx) inside a `use cache` function instead of the `fetch` `next.tags` option, then choose the invalidation API by the behavior you want:

- [`updateTag`](../03-api-reference/04-functions/updateTag.mdx): for mutations whose result the user must see immediately (read-your-own-writes). Called from a Server Action, it expires the tag so the next request waits for fresh data instead of serving stale content.
- [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx): for stale-while-revalidate. Pass a cache profile like `'max'` to serve cached data while it refreshes in the background. Works in Server Actions and Route Handlers.
- [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx): unchanged from the previous caching model.
...
**Not needed.** `unstable_noStore` (`noStore()`) opts a component out of caching. With Cache Components, nothing is cached unless you add `use cache`, so you can remove it. If a component must run at request time, call [`connection()`](../03-api-reference/04-functions/connection.mdx) before the work and wrap it in `<Suspense>`.
...
One behavior changes for [dynamic routes](../03-api-reference/03-file-conventions/dynamic-routes.mdx) when Cache Components is enabled.

### `generateStaticParams` must return at least one param

**Returning an empty array now errors.** Without Cache Components, returning `[]` defers every path to the first runtime visit. With Cache Components, [`generateStaticParams`](../03-api-reference/04-functions/generate-static-params.mdx) must return at least one param so Next.js can prerender the route. An empty array raises [`empty-generate-static-params`](/docs/messages/empty-generate-static-params).
...
**Wrap runtime data access in `<Suspense>`.** Without Cache Components, reading [`cookies()`](../03-api-reference/04-functions/cookies.mdx), [`headers()`](../03-api-reference/04-functions/headers.mdx), or [`searchParams`](../03-api-reference/03-file-conventions/page.mdx#searchparams-optional) opts the whole route into dynamic rendering. With Cache Components, accessing them outside a [`<Suspense>`](https://react.dev/reference/react/Suspense) boundary surfaces the [**blocking-route** insight](/docs/messages/blocking-route). Move the access into a component wrapped in `<Suspense>` so the rest of the page prerenders as a static shell and the dynamic part streams in at request time.
...
Your page receives `params` and `searchParams` as props, and both are promises. Apply the same pattern: pass the promise straight through to the `<Suspense>`-wrapped component as a prop and `await` it there, rather than at the top of the page. You can also unwrap the promise inline with `.then()` and pass a plain value down; see [Streaming](streaming.mdx#push-dynamic-access-down) for a similar pattern.
...
**Cache external data with `use cache`, or mark intentionally dynamic pages.** Under Cache Components, [`generateMetadata`](../03-api-reference/04-functions/generate-metadata.mdx) and [`generateViewport`](../03-api-reference/04-functions/generate-viewport.mdx) follow the same rules as components. If they read runtime data (`cookies()`, `headers()`, `params`, `searchParams`) or fetch uncached data while the rest of the page is otherwise prerenderable, Next.js raises an error so the choice is explicit. If the metadata depends on external but not runtime data, add `use cache`.
...
See [`generateMetadata` with Cache Components](../03-api-reference/04-functions/generate-metadata.mdx#with-cache-components) and [`generateViewport` with Cache Components](../03-api-reference/04-functions/generate-viewport.mdx#with-cache-components) for the full set of fix options and trade-offs.

## `runtime = 'edge'`

**Not supported.** Cache Components requires the Node.js runtime. Switch to the Node.js runtime (the default) by removing the `runtime = 'edge'` export. If you need edge behavior for specific routes, use [Proxy](../03-api-reference/03-file-conventions/proxy.mdx) instead.

## `experimental_ppr`

**Removed. Enable `cacheComponents` instead.** Next.js 16 removes the experimental Partial Prerendering flag (`experimental.ppr`) and the `experimental_ppr` route segment config. Partial Prerendering is now part of [Cache Components](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx), so remove `experimental.ppr` from `next.config` and `experimental_ppr` from your segments. A [codemod](upgrading/codemods.mdx#remove-experimental_ppr-route-segment-config-from-app-router-pages-and-layouts) removes the segment config for you.
...
See [Preserving UI state across navigations](preserving-ui-state.mdx) for detailed examples of each pattern.
