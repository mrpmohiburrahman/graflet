...
> - Inline scripts with `dangerouslySetInnerHTML` are blocked by strict [Content Security Policies](https://developer.mozilla.org/en-US/docs/Web/HTTP/CSP) that don't allow `'unsafe-inline'`. If your app uses a CSP, you'll need to add a [nonce](content-security-policy.mdx).
...
| Situation                                       | Approach                                                                                                                                                             |
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Date depends on request data (cookies, headers) | Use [`headers()`](../03-api-reference/04-functions/headers.mdx) or [`cookies()`](../03-api-reference/04-functions/cookies.mdx) to read request data and format server-side |
...
| Translating content between languages           | Use [internationalization](internationalization.mdx) with per-locale static builds or dynamic rendering                                                 |
...
- [Read cookies](../03-api-reference/04-functions/cookies.mdx) or [headers](../03-api-reference/04-functions/headers.mdx) when you need to format based on request data.
- [Configure internationalization](internationalization.mdx) for multi-language support.
