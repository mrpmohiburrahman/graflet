...
See the [`loading.js` API reference](../03-api-reference/03-file-conventions/loading.mdx) for more details.
...
If a component throws an error after streaming has started, the nearest [`error.js`](../03-api-reference/03-file-conventions/error.mdx) boundary catches it and renders the error UI in place of the failed component. The rest of the page remains intact, only the section that errored is replaced.
...
See [Sharing data with context and React.cache](../01-getting-started/06-fetching-data.mdx#sharing-data-with-context-and-reactcache) for the full pattern including the provider and consumer components.

## Streaming in Route Handlers

The patterns above rely on React and Suspense to stream UI. Outside of React rendering, [Route Handlers](../03-api-reference/03-file-conventions/route.mdx) can stream raw responses using the Web Streams API. This is useful for Server-Sent Events, large file generation, or any response where you want data to arrive progressively:
...
See the [Route Handler API reference](../03-api-reference/03-file-conventions/route.mdx) for more details on building streaming endpoints.
...
- Use the [`preload`](../03-api-reference/02-components/image.mdx#preload) prop on `next/image` for LCP images. This injects a `<link rel="preload">` into the `<head>`, so the browser starts fetching the image from the very first chunk, before the `<img>` tag even appears in the HTML.
...
When a `<Suspense>` fallback renders or a component suspends, the server must commit to `200 OK` in order to start sending the HTML stream. If a [`notFound()`](../03-api-reference/04-functions/not-found.mdx) fires mid-stream, Next.js cannot go back and change the status to 404. Instead, it injects `<meta name="robots" content="noindex">` into the streamed HTML so that search engines don't index the page. Similarly, a [`redirect()`](../03-api-reference/04-functions/redirect.mdx) mid-stream becomes a client-side redirect rather than an HTTP redirect header.
...
> **Good to know:** You can also reject requests early using [`proxy`](../03-api-reference/03-file-conventions/proxy.mdx) (for redirects, rewrites, or returning a response) or [`next.config.js` redirects](../03-api-reference/05-config/01-next-config-js/redirects.mdx). Both run before the page renders, so HTTP status codes are still available.

### Metadata and bots

[`generateMetadata`](../03-api-reference/04-functions/generate-metadata.mdx) resolves before streaming begins for bots that only scrape static HTML (such as Twitterbot or Slackbot). For full browsers and capable crawlers, metadata can [stream](../03-api-reference/04-functions/generate-metadata.mdx#streaming-metadata) alongside the page content.

Next.js automatically detects user agents to choose the right behavior. You can customize which bots receive blocking metadata with the [`htmlLimitedBots`](../03-api-reference/05-config/01-next-config-js/htmlLimitedBots.mdx) configuration option.

See the [`loading.js` SEO section](../03-api-reference/03-file-conventions/loading.mdx#seo) for more details.
...
| ------------------------------------------------------------------- | ----------------- |
| [Node.js server](../01-getting-started/17-deploying.mdx#nodejs-server) | Yes               |
| [Docker container](../01-getting-started/17-deploying.mdx#docker)      | Yes               |
| [Static export](../01-getting-started/17-deploying.mdx#static-export)  | No                |
| [Adapters](../01-getting-started/17-deploying.mdx#adapters)            | Platform-specific |

See the [Self-Hosting guide](self-hosting.mdx#streaming-and-suspense) for detailed configuration instructions.
...
- [Preventing flash before hydration](preventing-flash-before-hydration.mdx) - how to update server-rendered HTML with client-specific values (locale, theme, persisted state) before the browser paints
