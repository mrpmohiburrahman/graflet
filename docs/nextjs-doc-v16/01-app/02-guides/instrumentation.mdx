...
To set up instrumentation, create `instrumentation.ts|js` file in the **root directory** of your project (or inside the [`src`](../03-api-reference/03-file-conventions/src-folder.mdx) folder if using one).
...
Next.js calls `register` in all environments, so it's important to conditionally import any code that doesn't support specific runtimes (e.g. [Edge or Node.js](../03-api-reference/07-edge.mdx)). You can use the `NEXT_RUNTIME` environment variable to get the current environment:
