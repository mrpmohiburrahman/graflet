...
> This guide assumes you are **not** using [Cache Components](../01-getting-started/08-caching.mdx) which was introduced in version 16 under the [`cacheComponents` flag](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx).

## Caching `fetch` requests

By default, [`fetch`](../03-api-reference/04-functions/fetch.mdx) requests are not cached. You can cache individual requests by setting the `cache` option to `'force-cache'`.
...
See the [`fetch` API reference](../03-api-reference/04-functions/fetch.mdx) to learn more.
...
See the [`unstable_cache` API reference](../03-api-reference/04-functions/unstable_cache.mdx) to learn more.

### Route segment config

You can configure caching behavior at the route level by exporting config options from a [Page](../03-api-reference/03-file-conventions/page.mdx), [Layout](../03-api-reference/03-file-conventions/layout.mdx), or [Route Handler](../03-api-reference/03-file-conventions/route.mdx).
...
- **`'force-dynamic'`**: Force [dynamic rendering](../04-glossary.mdx#dynamic-rendering), which will result in routes being rendered for each user at request time. This option is equivalent to:
...
- **`'force-static'`**: Force prerendering and cache the data of a layout or page by forcing [`cookies`](../03-api-reference/04-functions/cookies.mdx), [`headers()`](../03-api-reference/04-functions/headers.mdx) and [`useSearchParams()`](../03-api-reference/04-functions/use-search-params.mdx) to return empty values. It is possible to [`revalidate`](#route-segment-config-revalidate), [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx), or [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx), in pages or layouts rendered with `force-static`.
...
To revalidate cached data after an event, use [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx) or [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx) in a [Server Action](../01-getting-started/07-mutating-data.mdx) or [Route Handler](../03-api-reference/03-file-conventions/route.mdx).
...
Invalidate cached data by tag using [`revalidateTag`](../03-api-reference/04-functions/revalidateTag.mdx):
...
Invalidate all cached data for a specific route path using [`revalidatePath`](../03-api-reference/04-functions/revalidatePath.mdx):
...
If you are not using `fetch` (which is [automatically memoized](../03-api-reference/04-functions/fetch.mdx#memoization)), and instead using an ORM or database directly, you can wrap your data access with the [React `cache`](https://react.dev/reference/react/cache) function to deduplicate requests within a single render pass:
