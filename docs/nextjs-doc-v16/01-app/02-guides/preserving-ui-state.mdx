...
> **Good to know:** This guide assumes [Cache Components](../01-getting-started/08-caching.mdx) is enabled. Enable it by setting [`cacheComponents: true`](../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) in your Next config file.

Before Cache Components, preserving page-level state across navigations required workarounds like hoisting state to a [shared layout](../01-getting-started/03-layouts-and-pages.mdx#nesting-layouts) or using an external store. With Cache Components, Next.js preserves state and DOM out of the box.
...
You can also use `Link`'s [`onNavigate`](../03-api-reference/02-components/link.mdx#onnavigate) callback to close dropdowns immediately when a navigation link is clicked.
