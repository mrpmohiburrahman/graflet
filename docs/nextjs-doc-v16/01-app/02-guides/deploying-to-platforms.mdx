...
**Functional fidelity** means every Next.js feature works correctly. The [adapter test suite](../03-api-reference/07-adapters/04-testing-adapters.mdx) is the contract: if a platform's adapter passes the tests, it supports Next.js. This is binary: it passes or it doesn't.
...
| ISR (on-demand)                | No        | Recommended  | No             | [Tag propagation](how-revalidation-works.mdx) needs shared cache for multi-instance |
| Partial Prerendering           | Required  | Recommended  | Optional       | [See PPR Platform Guide](ppr-platform-guide.mdx)                                    |
...
| `after()`                      | No        | No           | No             | Requires [graceful shutdown](self-hosting.mdx#after) support                        |
...
2. **Runs the compatibility test suite.** The platform provides a way to run the full [Next.js compatibility test suite](../03-api-reference/07-adapters/04-testing-adapters.mdx) against their adapter. This gives visibility into which features work, which are in progress, and where gaps remain.
