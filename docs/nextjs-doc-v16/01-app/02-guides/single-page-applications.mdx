...
The [`next/link`](../03-api-reference/02-components/link.mdx) component automatically [prefetches](../03-api-reference/02-components/link.mdx#prefetch) routes, giving you the fast page transitions of a strict SPA, but with the advantage of persisting application routing state to the URL for linking and sharing.

Next.js can start as a static site or even a strict SPA where everything is rendered client-side. If your project grows, Next.js allows you to progressively add more server features (e.g. [React Server Components](../01-getting-started/05-server-and-client-components.mdx), [Server Actions](../01-getting-started/07-mutating-data.mdx), and more) as needed.
...
While you can [defer and pass a single Promise](../01-getting-started/06-fetching-data.mdx#streaming-data-with-the-use-api) as a prop to a Client Component, we generally see this pattern paired with a React context provider. This enables easier access from Client Components with a custom React Hook.
...
Client components are [prerendered](https://github.com/reactwg/server-components/discussions/4) during `next build`. If you want to disable prerendering for a Client Component and only load it in the browser environment, you can use [`next/dynamic`](lazy-loading.mdx#nextdynamic):
...
If you are migrating from a strict SPA like [Create React App](migrating/from-create-react-app.mdx) or [Vite](migrating/from-vite.mdx), you might have existing code which shallow routes to update the URL state. This can be useful for manual transitions between views in your application _without_ using the default Next.js file-system routing.
...
Learn more about how [routing and navigation](../01-getting-started/04-linking-and-navigating.mdx#how-navigation-works) work in Next.js.
...
Learn more about [mutating data with Server Actions](../01-getting-started/07-mutating-data.mdx).
