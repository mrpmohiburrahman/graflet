...
Refer to the [`next/script`](../03-api-reference/02-components/script.mdx#strategy) API reference documentation to learn more about each strategy and their use cases.
...
If you would like to add additional configuration options, you can include it within the `<Head />` component used in a [custom `_document.js`](../../02-pages/03-building-your-application/01-routing/06-custom-document.mdx):
...
> **Note**: If you are using an [asset prefix](../../02-pages/04-api-reference/04-config/01-next-config-js/assetPrefix.mdx) and would like to modify Partytown's default configuration, you must include it as part of the `lib` path.
...
These handlers will only work when `next/script` is imported and used inside of a [Client Component](../01-getting-started/05-server-and-client-components.mdx) where `"use client"` is defined as the first line of code:
...
Refer to the [`next/script`](../03-api-reference/02-components/script.mdx#onload) API reference to learn more about each event handler and view examples.
...
Refer to the [`next/script`](../../02-pages/04-api-reference/01-components/script.mdx#onload) API reference to learn more about each event handler and view examples.
