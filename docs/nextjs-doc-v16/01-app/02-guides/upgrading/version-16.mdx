...
Learn more in the documentation [here](../mcp.mdx).

### Using the Codemod

To update to Next.js version 16, you can use the `upgrade` [codemod](codemods.mdx#160):
...
The [codemod](codemods.mdx#160) is able to:
...
If your project has a [custom `webpack`](../../03-api-reference/05-config/01-next-config-js/webpack.mdx) configuration and you run `next build` (which now uses Turbopack by default), the build will **fail** to prevent misconfiguration issues.
...
Make sure to review the `Turbopack` configuration [options](../../03-api-reference/05-config/01-next-config-js/turbopack.mdx). **Next.js 16** introduces various improvements and new options, for example:

- [Advanced Webpack loader conditions](../../03-api-reference/05-config/01-next-config-js/turbopack.mdx#advanced-webpack-loader-conditions)
- [debugIds](../../03-api-reference/05-config/01-next-config-js/turbopack.mdx#debug-ids)
...
- [`cookies`](../../03-api-reference/04-functions/cookies.mdx)
- [`headers`](../../03-api-reference/04-functions/headers.mdx)
- [`draftMode`](../../03-api-reference/04-functions/draft-mode.mdx)
- `params` in [`layout.js`](../../03-api-reference/03-file-conventions/layout.mdx), [`page.js`](../../03-api-reference/03-file-conventions/page.mdx), [`route.js`](../../03-api-reference/03-file-conventions/route.mdx), [`default.js`](../../03-api-reference/03-file-conventions/default.mdx), [`opengraph-image`](../../03-api-reference/03-file-conventions/01-metadata/opengraph-image.mdx#opengraph-image), [`twitter-image`](../../03-api-reference/03-file-conventions/01-metadata/opengraph-image.mdx#twitter-image), [`icon`](../../03-api-reference/03-file-conventions/01-metadata/app-icons.mdx#icon), and [`apple-icon`](../../03-api-reference/03-file-conventions/01-metadata/app-icons.mdx#apple-icon).
- `searchParams` in [`page.js`](../../03-api-reference/03-file-conventions/page.mdx)

Use the [codemod](codemods.mdx#migrate-to-async-dynamic-apis) to migrate to async Request-time APIs.

### Migrating types for async Request-time APIs

To help migrate to async `params` and `searchParams`, you can run [`npx next typegen`](../../03-api-reference/06-cli/next.mdx#next-typegen-options) to automatically generate these globally available types helpers:

- [`PageProps`](../../03-api-reference/03-file-conventions/page.mdx#page-props-helper)
- [`LayoutProps`](../../03-api-reference/03-file-conventions/layout.mdx#layout-props-helper)
- [`RouteContext`](../../03-api-reference/03-file-conventions/route.mdx#route-context-helper)
...
Previously, the `id` values returned from [`generateSitemaps`](../../03-api-reference/04-functions/generate-sitemaps.mdx) were passed directly to the `sitemap` generating function.
...
[`revalidateTag`](../../03-api-reference/04-functions/revalidateTag.mdx) now requires a second argument specifying a [`cacheLife`](../../03-api-reference/04-functions/cacheLife.mdx#reference) profile. The single-argument form is deprecated and will produce a TypeScript error.
...
If you need immediate expiration rather than stale-while-revalidate, use [`updateTag`](../../03-api-reference/04-functions/updateTag.mdx) in Server Actions instead.
...
[`updateTag`](../../03-api-reference/04-functions/updateTag.mdx) is a new [Server Actions](../../01-getting-started/07-mutating-data.mdx#what-are-server-functions)-only API that provides **read-your-writes** semantics, where a user makes a change and the UI immediately shows the change, rather than stale data.
...
Learn more about when to use `updateTag` or `revalidateTag` [here](../../03-api-reference/04-functions/updateTag.mdx#when-to-use-updatetag).

### refresh

[`refresh`](../../03-api-reference/04-functions/refresh.mdx) allows you to refresh the client router from within a Server Action.
...
[`cacheLife`](../../03-api-reference/04-functions/cacheLife.mdx) and [`cacheTag`](../../03-api-reference/04-functions/cacheTag.mdx) are now stable. The `unstable_` prefix is no longer needed.
...
Starting with **Next.js 16**, you can opt into PPR using the [`cacheComponents`](../../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) configuration.
...
PPR in **Next.js 16** works differently than in **Next.js 15** canaries. If you are using PPR today, stay in the current Next.js 15 canary you are using. See [Migrating to Cache Components](../migrating-to-cache-components.mdx) for migration patterns.
...
The version 16 [codemod](codemods.mdx#160) is able to update these flags too.
...
All [parallel route](../../03-api-reference/03-file-conventions/parallel-routes.mdx) slots now require explicit `default.js` files. Builds will fail without them.

To maintain previous behavior, create a [`default.js`](../../03-api-reference/03-file-conventions/default.mdx) file that calls `notFound()` or returns `null`.
...
Make sure to review our API reference for the [`@next/eslint-plugin-next`](../../03-api-reference/05-config/03-eslint.mdx#setup-eslint) plugin.
...
Alternatively, use the [`phase`](../../03-api-reference/05-config/01-next-config-js/index.mdx#phase) in which the configuration is loaded.
...
`adapterPath` was promoted to a stable, top-level option in 16.2.0. See [`adapterPath`](../../03-api-reference/05-config/01-next-config-js/adapterPath.mdx) for the current API reference.
...
> **Good to know**: Use the [taint API](../../03-api-reference/05-config/01-next-config-js/taint.mdx) to prevent accidentally passing sensitive server values to Client Components.
...
To ensure environment variables are read at runtime (not bundled at build time), use the [`connection()`](../../03-api-reference/04-functions/connection.mdx) function before reading from `process.env`:
...
The following options have been removed from [`devIndicators`](../../03-api-reference/05-config/01-next-config-js/devIndicators.mdx):
...
The `experimental.dynamicIO` and `experimental.useCache` flags are deprecated. Use top-level [`cacheComponents`](../../03-api-reference/05-config/01-next-config-js/cacheComponents.mdx) instead.
...
For more on Cache Components, see [Migrating to Cache Components](../migrating-to-cache-components.mdx).
