...
You can use the [`<form>`](https://react.dev/reference/react-dom/components/form) element with React's [Server Actions](../01-getting-started/07-mutating-data.mdx) and `useActionState` to capture user credentials, validate form fields, and call your Authentication Provider's API or database.
...
1. Generate a secret key, which will be used to sign your session, and store it as an [environment variable](environment-variables.mdx).
2. Write logic to encrypt/decrypt session data using a session management library.
3. Manage cookies using the Next.js [`cookies`](../03-api-reference/04-functions/cookies.mdx) API.
...
This command generates a 32-character random string that you can use as your secret key and store in your [environment variables file](environment-variables.mdx):
...
To store the session in a cookie, use the Next.js [`cookies`](../03-api-reference/04-functions/cookies.mdx) API. The cookie should be set on the server, and include the recommended options:
...
Back in your Server Action, you can invoke the `createSession()` function, and use the [`redirect()`](redirecting.mdx) API to redirect the user to the appropriate page:
...
You can use [API Routes](../../02-pages/03-building-your-application/01-routing/07-api-routes.mdx) to set the session as a cookie on the server:
...
There are some cases where you may want to use [Proxy](../03-api-reference/03-file-conventions/proxy.mdx) and redirect users based on permissions:
...
However, since Proxy runs on every route, including [prefetched](../01-getting-started/04-linking-and-navigating.mdx#prefetching) routes, it's important to only read the session from the cookie (optimistic checks), and avoid database checks to prevent performance issues.
...
Auth check in [Server Components](../01-getting-started/05-server-and-client-components.mdx) are useful for role-based access. For example, to conditionally render components based on the user's role:
...
Due to [Partial Rendering](../01-getting-started/04-linking-and-navigating.mdx#client-side-transitions), be cautious when doing checks in [Layouts](../03-api-reference/03-file-conventions/layout.mdx) as these don't re-render on navigation, meaning the user session won't be checked on every route change.
...
Treat [Server Actions](../01-getting-started/07-mutating-data.mdx) with the same security considerations as public-facing API endpoints, and verify if the user is allowed to perform a mutation.
...
Treat [Route Handlers](../03-api-reference/03-file-conventions/route.mdx) with the same security considerations as public-facing API endpoints, and verify if the user is allowed to access the Route Handler.
...
Using context providers for auth works due to [interleaving](../01-getting-started/05-server-and-client-components.mdx#interleaving-server-and-client-components). However, React `context` is not supported in Server Components, making them only applicable to Client Components.
