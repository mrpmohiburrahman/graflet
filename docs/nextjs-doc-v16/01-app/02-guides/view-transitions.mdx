...
- [View transition configuration](../03-api-reference/05-config/01-next-config-js/viewTransition.mdx)
- [Link `transitionTypes` prop](../03-api-reference/02-components/link.mdx#transitiontypes)
- [`useRouter`](../03-api-reference/04-functions/use-router.mdx), which also supports `transitionTypes` in `push()` and `replace()`
