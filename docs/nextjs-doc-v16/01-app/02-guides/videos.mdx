...
The first step is to create a [Server Component](../01-getting-started/05-server-and-client-components.mdx) that generates the appropriate iframe for embedding the video. This component will fetch the source URL for the video and render the iframe.
...
After creating the Server Component to embed the video, the next step is to [stream](../03-api-reference/03-file-conventions/loading.mdx) the component using [React Suspense](https://react.dev/reference/react/Suspense).
