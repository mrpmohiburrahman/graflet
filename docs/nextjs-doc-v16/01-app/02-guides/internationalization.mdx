...
Routing can be internationalized by either the sub-path (`/fr/products`) or domain (`my-site.fr/products`). With this information, you can now redirect the user based on the locale inside [Proxy](../03-api-reference/03-file-conventions/proxy.mdx).
...
> **Good to know:** `PageProps` and `LayoutProps` are globally available TypeScript helpers that provide strong typing for route parameters. See [PageProps](../03-api-reference/03-file-conventions/page.mdx#page-props-helper) and [LayoutProps](../03-api-reference/03-file-conventions/layout.mdx#layout-props-helper) for more details.
...
Because all layouts and pages in the `app/` directory default to [Server Components](../01-getting-started/05-server-and-client-components.mdx), we do not need to worry about the size of the translation files affecting our client-side JavaScript bundle size. This code will **only run on the server**, and only the resulting HTML will be sent to the browser.
