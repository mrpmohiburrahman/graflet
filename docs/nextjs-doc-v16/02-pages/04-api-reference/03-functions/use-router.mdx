...
- `query`: `Object` - The query string parsed to an object, including [dynamic route](../../03-building-your-application/01-routing/02-dynamic-routes.mdx) parameters. It will be an empty object during prerendering if the page doesn't use [Server-side Rendering](../../03-building-your-application/03-data-fetching/03-get-server-side-props.mdx). Defaults to `{}`
...
- `isFallback`: `boolean` - Whether the current page is in [fallback mode](get-static-paths.mdx#fallback-true).
- `basePath`: `String` - The active [basePath](../../../01-app/03-api-reference/05-config/01-next-config-js/basePath.mdx) (if enabled).
- `locale`: `String` - The active locale (if enabled).
- `locales`: `String[]` - All supported locales (if enabled).
- `defaultLocale`: `String` - The current default locale (if enabled).
- `domainLocales`: `Array<{domain, defaultLocale, locales}>` - Any configured domain locales.
- `isReady`: `boolean` - Whether the router fields are updated client-side and ready for use. Should only be used inside of `useEffect` methods and not for conditionally rendering on the server. See related docs for use case with [automatically statically optimized pages](../../03-building-your-application/02-rendering/04-automatic-static-optimization.mdx)
- `isPreview`: `boolean` - Whether the application is currently in [preview mode](../../02-guides/preview-mode.mdx).

> Using the `asPath` field may lead to a mismatch between client and server if the page is rendered using server-side rendering or [automatic static optimization](../../03-building-your-application/02-rendering/04-automatic-static-optimization.mdx). Avoid using `asPath` until the `isReady` field is `true`.

The following methods are included inside `router`:

### router.push

Handles client-side transitions, this method is useful for cases where [`next/link`](../01-components/link.mdx) is not enough.
...
- `options` - Optional object with the following configuration options:
  ...
  - [`shallow`](../../03-building-your-application/01-routing/03-linking-and-navigating.mdx#shallow-routing): Update the path of the current page without rerunning [`getStaticProps`](../../03-building-your-application/03-data-fetching/01-get-static-props.mdx), [`getServerSideProps`](../../03-building-your-application/03-data-fetching/03-get-server-side-props.mdx) or [`getInitialProps`](get-initial-props.mdx). Defaults to `false`
...
Redirecting the user to `pages/login.js`, useful for pages behind [authentication](../../02-guides/authentication.mdx):
...
You can use a URL object in the same way you can use it for [`next/link`](../01-components/link.mdx#passing-a-url-object). Works for both the `url` and `as` parameters:
...
Similar to the `replace` prop in [`next/link`](../01-components/link.mdx), `router.replace` will prevent adding a new URL entry into the `history` stack.
...
Prefetch pages for faster client-side transitions. This method is only useful for navigations without [`next/link`](../01-components/link.mdx), as `next/link` takes care of prefetching pages automatically.
...
In some cases (for example, if using a [Custom Server](../../02-guides/custom-server.mdx)), you may wish to listen to [popstate](https://developer.mozilla.org/docs/Web/API/Window/popstate_event) and do something before the router acts on it.
...
If `cb` returns `false`, the Next.js router will not handle `popstate`, and you'll be responsible for handling it in that case. See [Disabling file-system routing](../../02-guides/custom-server.mdx#disabling-file-system-routing).
...
> **Good to know**: Here `url` is the URL shown in the browser, including the [`basePath`](../../../01-app/03-api-reference/05-config/01-next-config-js/basePath.mdx).
...
> We use a [Custom App](../../03-building-your-application/01-routing/05-custom-app.mdx) (`pages/_app.js`) for this example to subscribe to the event because it's not unmounted on page navigations, but you can subscribe to router events on any component in your application.
