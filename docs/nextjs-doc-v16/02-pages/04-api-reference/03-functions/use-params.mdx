...
`useParams` is a hook that lets you read a route's [dynamic params](../../03-building-your-application/01-routing/02-dynamic-routes.mdx) filled in by the current URL.
...
`useParams` returns an object containing the current route's filled in [dynamic parameters](../../03-building-your-application/01-routing/02-dynamic-routes.mdx), or `null` during [prerendering](#behavior-during-prerendering).

- Each property in the object is an active dynamic segment.
- The property name is the segment's name, and the property value is what the segment is filled in with.
- The property value will either be a `string` or array of `string`s depending on the [type of dynamic segment](../../03-building-your-application/01-routing/02-dynamic-routes.mdx).
...
For pages that are [statically optimized](../../03-building-your-application/02-rendering/04-automatic-static-optimization.mdx), `useParams` will return `null` on the initial render. After hydration, the value will be updated to the actual params once the router is ready.
...
When using [`getServerSideProps`](../../03-building-your-application/03-data-fetching/03-get-server-side-props.mdx), the page is server-rendered on each request and `useParams` will return the actual params immediately:
...
`useParams` only returns the dynamic route parameters, whereas [`router.query`](use-router.mdx#router-object) from `useRouter` includes both dynamic parameters and query string parameters.
