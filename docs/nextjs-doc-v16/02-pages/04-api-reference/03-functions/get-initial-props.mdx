...
> **Good to know**: `getInitialProps` is a legacy API. We recommend using [`getStaticProps`](../../03-building-your-application/03-data-fetching/01-get-static-props.mdx) or [`getServerSideProps`](../../03-building-your-application/03-data-fetching/03-get-server-side-props.mdx) instead.
...
> - For the initial page load, `getInitialProps` will run on the server only. `getInitialProps` will then also run on the client when navigating to a different route with the [`next/link`](../01-components/link.mdx) component or by using [`next/router`](use-router.mdx).
...
- `getInitialProps` can only be used in `pages/` top level files, and not in nested components. To have nested data fetching at the component level, consider exploring the [App Router](../../../01-app/01-getting-started/06-fetching-data.mdx).
