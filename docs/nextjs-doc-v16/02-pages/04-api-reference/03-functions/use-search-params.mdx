...
For pages that are [statically optimized](../../03-building-your-application/02-rendering/04-automatic-static-optimization.mdx) (not using `getServerSideProps`), `useSearchParams` will return `null` during prerendering. After hydration, the value will be updated to the actual search params.
...
When using [`getServerSideProps`](../../03-building-your-application/03-data-fetching/03-get-server-side-props.mdx), the page is server-rendered on each request and `useSearchParams` will return the actual search params immediately:
...
You can use the [`useRouter`](use-router.mdx) hook to update search params:
...
> **Good to know**: When using this component in the App Router, wrap it in a `<Suspense>` boundary for [prerendering](../../../01-app/03-api-reference/04-functions/use-search-params.mdx#prerendering) support.
