...
When exporting a function called `getStaticPaths` from a page that uses [Dynamic Routes](../../03-building-your-application/01-routing/02-dynamic-routes.mdx), Next.js will statically prerender all the paths specified by `getStaticPaths`.
...
The `paths` key determines which paths will be prerendered. For example, suppose that you have a page that uses [Dynamic Routes](../../03-building-your-application/01-routing/02-dynamic-routes.mdx) named `pages/posts/[id].js`. If you export `getStaticPaths` from this page and return the following for `paths`:
...
- If the page name uses [catch-all routes](../../03-building-your-application/01-routing/02-dynamic-routes.mdx#catch-all-segments) like `pages/[...slug]`, then `params` should contain `slug` (which is an array). If this array is `['hello', 'world']`, then Next.js will statically generate the page at `/hello/world`.
- If the page uses an [optional catch-all route](../../03-building-your-application/01-routing/02-dynamic-routes.mdx#optional-catch-all-segments), use `null`, `[]`, `undefined` or `false` to render the root-most route. For example, if you supply `slug: false` for `pages/[[...slug]]`, Next.js will statically generate the page `/`.
...
Separate of the `params` object a `locale` field can be returned when [i18n is configured](../../02-guides/internationalization.mdx), which configures the locale for the path being generated.
...
> **Good to know**: `fallback: true` is not supported when using [`output: 'export'`](../../02-guides/static-exports.mdx).
...
`fallback: true` will not _update_ generated pages, for that take a look at [Incremental Static Regeneration](../../02-guides/incremental-static-regeneration.mdx).
...
`fallback: 'blocking'` will not _update_ generated pages by default. To update generated pages, use [Incremental Static Regeneration](../../02-guides/incremental-static-regeneration.mdx) in conjunction with `fallback: 'blocking'`.

> **Good to know**: `fallback: 'blocking'` is not supported when using [`output: 'export'`](../../02-guides/static-exports.mdx).
...
- Using the [router](use-router.mdx), you can detect if the fallback is being rendered, `router.isFallback` will be `true`.
...
| Version   | Changes                                                                                                                                                                                           |
| --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v13.4.0` | [App Router](../../../01-app/01-getting-started/06-fetching-data.mdx) is now stable with simplified data fetching, including [`generateStaticParams()`](../../../01-app/03-api-reference/04-functions/generate-static-params.mdx) |
| `v12.2.0` | [On-Demand Incremental Static Regeneration](../../02-guides/incremental-static-regeneration.mdx#on-demand-revalidation-with-revalidatepath) is stable.                                             |
| `v12.1.0` | [On-Demand Incremental Static Regeneration](../../02-guides/incremental-static-regeneration.mdx#on-demand-revalidation-with-revalidatepath) added (beta).                                          |
| `v9.5.0`  | Stable [Incremental Static Regeneration](../../02-guides/incremental-static-regeneration.mdx)                                                                                                      |
