...
With Next.js 12 and [Proxy](../04-api-reference/02-file-conventions/proxy.mdx), we can add a prefix to the default locale with a [workaround](https://github.com/vercel/next.js/discussions/18419).
...
Next, we can use [Proxy](../04-api-reference/02-file-conventions/proxy.mdx) to add custom routing rules:
...
This [Proxy](../04-api-reference/02-file-conventions/proxy.mdx) skips adding the default prefix to [API Routes](../03-building-your-application/01-routing/07-api-routes.mdx) and [public](../04-api-reference/02-file-conventions/public-folder.mdx) files like fonts or images. If a request is made to the default locale, we redirect to our prefix `/en`.
...
You can access the locale information via the Next.js router. For example, using the [`useRouter()`](../04-api-reference/03-functions/use-router.mdx) hook the following properties are available:

- `locale` contains the currently active locale.
- `locales` contains all configured locales.
- `defaultLocale` contains the configured default locale.

When [prerendering](../03-building-your-application/02-rendering/02-static-site-generation.mdx) pages with `getStaticProps` or `getServerSideProps`, the locale information is provided in [the context](../03-building-your-application/03-data-fetching/01-get-static-props.mdx) provided to the function.
...
Note that to handle switching only the `locale` while preserving all routing information such as [dynamic route](../03-building-your-application/01-routing/02-dynamic-routes.mdx) query values or hidden href query values, you can provide the `href` parameter as an object:
...
See [here](../04-api-reference/03-functions/use-router.mdx#with-url-object) for more information on the object structure for `router.push`.
...
Next.js doesn't know about variants of a page so it's up to you to add the `hreflang` meta tags using [`next/head`](../04-api-reference/01-components/head.mdx). You can learn more about `hreflang` in the [Google Webmasters documentation](https://support.google.com/webmasters/answer/189077).

## How does this work with Static Generation?

> Note that Internationalized Routing does not integrate with [`output: 'export'`](static-exports.mdx) as it does not leverage the Next.js routing layer. Hybrid Next.js applications that do not use `output: 'export'` are fully supported.

### Dynamic Routes and `getStaticProps` Pages

For pages using `getStaticProps` with [Dynamic Routes](../03-building-your-application/01-routing/02-dynamic-routes.mdx), all locale variants of the page desired to be prerendered need to be returned from [`getStaticPaths`](../03-building-your-application/03-data-fetching/02-get-static-paths.mdx). Along with the `params` object returned for `paths`, you can also return a `locale` field specifying which locale you want to render. For example:
...
To decrease the build time of dynamic pages with `getStaticProps`, use a [`fallback` mode](../04-api-reference/03-functions/get-static-paths.mdx#fallback-true). This allows you to return only the most popular paths and locales from `getStaticPaths` for prerendering during the build. Then, Next.js will build the remaining pages at runtime as they are requested.

### Automatically Statically Optimized Pages

For pages that are [automatically statically optimized](../03-building-your-application/02-rendering/04-automatic-static-optimization.mdx), a version of the page will be generated for each locale.
...
> **Good to know**: These limits have been added initially to prevent potential [performance issues at build time](#dynamic-routes-and-getstaticprops-pages). You can workaround these limits with custom routing using [Proxy](../04-api-reference/02-file-conventions/proxy.mdx) in Next.js 12.
