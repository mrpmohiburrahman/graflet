...
> **Note**: This feature is superseded by [Draft Mode](draft-mode.mdx).
...
In the [Pages documentation](../03-building-your-application/01-routing/01-pages-and-layouts.mdx) and the [Data Fetching documentation](../03-building-your-application/03-data-fetching/index.mdx), we talked about how to prerender a page at build time (**Static Generation**) using `getStaticProps` and `getStaticPaths`.
...
> Take a look at the [API Routes documentation](../03-building-your-application/01-routing/07-api-routes.mdx) first if you’re not familiar with Next.js API Routes.
...
> **Good to know**: during rendering `next/router` exposes an `isPreview` flag, see the [router object docs](../04-api-reference/03-functions/use-router.mdx#router-object) for more info.
...
Then, send a request to `/api/clear-preview-mode-cookies` to invoke the API Route. If calling this route using [`next/link`](../04-api-reference/01-components/link.mdx), you must pass `prefetch={false}` to prevent calling `clearPreviewData` during link prefetching.
...
> **Good to know**: You shouldn't set the `Cache-Control` header when using Preview Mode because it cannot be bypassed. Instead, we recommend using [ISR](incremental-static-regeneration.mdx).
