...
> - Since API Routes run on the server, we're able to use sensitive values (like API keys) through [Environment Variables](environment-variables.mdx) without exposing them to the client. This is critical for the security of your application.
...
If you would like to redirect the user to a different route after a mutation, you can [`redirect`](../03-building-your-application/01-routing/07-api-routes.mdx#response-helpers) to any absolute or relative URL:
