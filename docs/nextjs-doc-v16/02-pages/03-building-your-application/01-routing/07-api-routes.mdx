...
> **Good to know**: If you are using the App Router, you can use [Server Components](../../../01-app/01-getting-started/05-server-and-client-components.mdx) or [Route Handlers](../../../01-app/03-api-reference/03-file-conventions/route.mdx) instead of API Routes.
...
> - API Routes can't be used with [static exports](../../02-guides/static-exports.mdx). However, [Route Handlers](../../../01-app/03-api-reference/03-file-conventions/route.mdx) in the App Router can.
> - API Routes will be affected by [`pageExtensions` configuration](../../04-api-reference/04-config/01-next-config-js/pageExtensions.mdx) in `next.config.js`.
...
- `res.revalidate(urlPath)` - [Revalidate a page on demand](../../02-guides/incremental-static-regeneration.mdx#on-demand-revalidation-with-revalidatepath) using `getStaticProps`. `urlPath` must be a `string`.
...
API Routes support [dynamic routes](02-dynamic-routes.mdx), and follow the same file naming rules used for `pages/`.
...
While the Pages Router does support streaming responses with API Routes, we recommend incrementally adopting the App Router and using [Route Handlers](../../../01-app/03-api-reference/03-file-conventions/route.mdx) if you are on Next.js 14+.
