...
> To learn more about dynamic routing, check the [Dynamic Routing documentation](02-dynamic-routes.mdx).

## Layout Pattern

The React model allows us to deconstruct a [page](01-pages-and-layouts.mdx) into a series of components. Many of these components are often reused between pages. For example, you might have the same navigation bar and footer on every page.
...
If you only have one layout for your entire application, you can create a [Custom App](05-custom-app.mdx) and wrap your application with the layout. Since the `<Layout />` component is re-used when changing pages, its component state will be preserved (e.g. input values).
...
Inside your layout, you can fetch data on the client-side using `useEffect` or a library like [SWR](https://swr.vercel.app/). Because this file is not a [Page](01-pages-and-layouts.mdx), you cannot use `getStaticProps` or `getServerSideProps` currently.
