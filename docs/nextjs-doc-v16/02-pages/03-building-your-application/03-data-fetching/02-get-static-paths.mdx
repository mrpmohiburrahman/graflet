...
If a page has [Dynamic Routes](../01-routing/02-dynamic-routes.mdx) and uses `getStaticProps`, it needs to define a list of paths to be statically generated.
...
The [`getStaticPaths` API reference](../../04-api-reference/03-functions/get-static-paths.mdx) covers all parameters and props that can be used with `getStaticPaths`.
...
- You **cannot** use `getStaticPaths` with [`getServerSideProps`](03-get-server-side-props.mdx)
- You can export `getStaticPaths` from a [Dynamic Route](../01-routing/02-dynamic-routes.mdx) that also uses `getStaticProps`
...
`getStaticPaths` allows you to control which pages are generated during the build instead of on-demand with [`fallback`](../../04-api-reference/03-functions/get-static-paths.mdx#fallback-blocking). Generating more pages during a build will cause slower builds.
