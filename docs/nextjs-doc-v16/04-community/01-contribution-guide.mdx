...
For example, in the [functions API Reference](../01-app/03-api-reference/04-functions/index.mdx), the pages are sorted alphabetically because it makes it easier for developers to find a specific function:
...
But, in the [app router section](../01-app/index.mdx), the files are prefixed with a two-digit number, sorted in the order developers should learn these concepts:
