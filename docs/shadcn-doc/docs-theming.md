...
Dark mode works by overriding the same tokens inside a `.dark` selector. See the [dark mode docs](docs-dark-mode-next.md) for adding a theme provider and toggling the `.dark` class.
...
The chart tokens are covered in more detail in the [Chart theming docs](docs-components-chart.md#theming).
