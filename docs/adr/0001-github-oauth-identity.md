...
- The CLI uses GitHub's device/browser OAuth flow; the **backend holds the client secret** (never shipped in the
  OSS CLI). The CLI never holds the maintainer's private-repo token.
