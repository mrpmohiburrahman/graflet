...
* Effect Events can only be called from inside Effects or other Effect Events. Do not call them during rendering or pass them to other components or Hooks. The [`eslint-plugin-react-hooks`](reference-eslint-plugin-react-hooks.md) linter enforces this restriction.
...
See [Separating Events from Effects](learn-separating-events-from-effects.md) to learn more.
