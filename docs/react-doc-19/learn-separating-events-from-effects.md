...
Let's say you've already implemented the code for them, but you're not sure where to put it. Should you use event handlers or Effects? Every time you need to answer this question, consider [*why* the code needs to run.](learn-synchronizing-with-effects.md#what-are-effects-and-how-are-they-different-from-events)
...
With this code, you can be sure that there is always an active connection to the currently selected chat server, *regardless* of the specific interactions performed by the user. Whether the user has only opened your app, selected a different room, or navigated to another screen and back, your Effect ensures that the component will *remain synchronized* with the currently selected room, and will [re-connect whenever it's necessary.](learn-lifecycle-of-reactive-effects.md#why-synchronization-may-need-to-happen-more-than-once)
...
- **Logic inside Effects is *reactive.*** If your Effect reads a reactive value, [you have to specify it as a dependency.](learn-lifecycle-of-reactive-effects.md#effects-react-to-reactive-values) Then, if a re-render causes that value to change, React will re-run your Effect's logic with the new value.
...
However, `theme` is a reactive value (it can change as a result of re-rendering), and [every reactive value read by an Effect must be declared as its dependency.](learn-lifecycle-of-reactive-effects.md#react-verifies-that-you-specified-every-reactive-value-as-a-dependency) Now you have to specify `theme` as a dependency of your Effect:
...
Use a special Hook called [`useEffectEvent`](reference-react-useEffectEvent.md) to extract this non-reactive logic out of your Effect:
...
Read [Removing Effect Dependencies](learn-removing-effect-dependencies.md) for other correct alternatives to suppressing the linter.
