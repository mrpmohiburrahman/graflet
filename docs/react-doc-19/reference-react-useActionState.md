...
`useActionState` is a React Hook that lets you update state with side effects using [Actions](reference-react-useTransition.md#functions-called-in-starttransition-are-called-actions).
...
* **optional** `permalink`: A string containing the unique page URL that this form modifies.
  * For use on pages with [React Server Components](reference-rsc-server-components.md) with progressive enhancement.
  * If `reducerAction` is a [Server Function](reference-rsc-server-functions.md) and the form is submitted before the JavaScript bundle loads, the browser will navigate to the specified permalink URL rather than the current page's URL.
...
2. A `dispatchAction` function that you call inside [Actions](reference-react-useTransition.md#functions-called-in-starttransition-are-called-actions).
...
* When using Server Functions, `initialState` needs to be [serializable](reference-rsc-use-server.md#serializable-parameters-and-return-values) (values like plain objects, arrays, strings, and numbers).
* If `dispatchAction` throws an error, React cancels all queued actions and shows the nearest [Error Boundary](reference-react-Component.md#catching-rendering-errors-with-an-error-boundary).
...
You can wrap it in [`startTransition`](reference-react-startTransition.md), or pass it to an [Action prop](reference-react-useTransition.md#exposing-action-props-from-components). Calls outside that scope won’t be treated as part of the Transition and [log an error](#async-function-outside-transition) on development mode.
...
* If you set state after `await` in the `reducerAction` you currently need to wrap the state update in an additional `startTransition`. See the [startTransition](reference-react-useTransition.md#react-doesnt-treat-my-state-update-after-await-as-a-transition) docs for more info.
* When using Server Functions, `actionPayload` needs to be [serializable](reference-rsc-use-server.md#serializable-parameters-and-return-values) (values like plain objects, arrays, strings, and numbers).
...
You can typically solve this by [using with useOptimistic](reference-react-useActionState.md#using-with-useoptimistic) but for more complex cases you may want to consider [cancelling queued actions](#cancelling-queued-actions) or not using `useActionState`.
...
You can think of `useActionState` as `useReducer` for side effects from user Actions. Since it computes the next Action to take based on the previous Action, it has to [order the calls sequentially](reference-react-useActionState.md#how-useactionstate-queuing-works). If you want to perform Actions in parallel, use `useState` and `useTransition` directly.
...
You can combine `useActionState` with [`useOptimistic`](reference-react-useOptimistic.md) to show immediate UI feedback:
...
When you pass the `dispatchAction` function to a component that exposes an [Action prop](reference-react-useTransition.md#exposing-action-props-from-components), you don't need to call `startTransition` or `useOptimistic` yourself.
...
When used with a [Server Function](reference-rsc-server-functions.md), `useActionState` allows the server's response to be shown before hydration (when React attaches to server-rendered HTML) completes. You can also use the optional `permalink` parameter for progressive enhancement (allowing the form to work before JavaScript loads) on pages with dynamic content. This is typically handled by your framework for you.

</RSC>

See the [`<form>`](reference-react-dom-components-form.md#handle-form-submission-with-a-server-function) docs for more information on using Actions with forms.
...
For unknown errors, such as `undefined is not a function`, you can throw an error. React will cancel all queued Actions and shows the nearest [Error Boundary](reference-react-Component.md#catching-rendering-errors-with-an-error-boundary) by rethrowing the error from the `useActionState` hook.
...
If you're calling `dispatchAction` manually (not through an Action prop), make sure you wrap the call in [`startTransition`](reference-react-startTransition.md):
...
To fix, either wrap the call in [`startTransition`](reference-react-startTransition.md):
