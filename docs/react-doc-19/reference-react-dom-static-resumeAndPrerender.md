...
This API depends on [Web Streams.](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) For Node.js, use [`resumeAndPrerenderToNodeStream`](reference-react-dom-static-resumeAndPrerenderToNodeStream.md) instead.
...
- If rendering the is successful, the Promise will resolve to an object containing:
  ...
  - `postponed`: an JSON-serializeable, opaque object that can be passed to [`resume`](reference-react-dom-server-resume.md) or [`resumeAndPrerender`](reference-react-dom-static-resumeAndPrerender.md) if `prerender` is aborted.
...
`resumeAndPrerender` behaves similarly to [`prerender`](reference-react-dom-static-prerender.md) but can be used to continue a previously started prerendering process that was aborted.
