...
* The effect of the directive depends on your [`compilationMode`](reference-react-compiler-compilationMode.md) setting.

### How `"use memo"` marks functions for optimization {/*how-use-memo-marks*/}

In a React app that uses the React Compiler, functions are analyzed at build time to determine if they can be optimized. By default, the compiler automatically infers which components to memoize, but this can depend on your [`compilationMode`](reference-react-compiler-compilationMode.md) setting if you've set it.
...
* [`"use no memo"`](reference-react-compiler-directives-use-no-memo.md) - Opt out of compilation
* [`compilationMode`](reference-react-compiler-compilationMode.md) - Configure compilation behavior
* [React Compiler](learn-react-compiler.md) - Getting started guide
---
