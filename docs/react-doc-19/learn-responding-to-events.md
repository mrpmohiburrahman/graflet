...
To add an event handler, you will first define a function and then [pass it as a prop](learn-passing-props-to-a-component.md) to the appropriate JSX tag. For example, here is a button that doesn't do anything yet:
...
You defined the `handleClick` function and then [passed it as a prop](learn-passing-props-to-a-component.md) to `<button>`.  `handleClick` is an **event handler.** Event handler functions:
...
In the second example, the `()` at the end of `handleClick()` fires the function *immediately* during [rendering](learn-render-and-commit.md), without any clicks. This is because JavaScript inside the [JSX `{` and `}`](learn-javascript-in-jsx-with-curly-braces.md) executes right away.
...
Built-in components like `<button>` and `<div>` only support [browser event names](reference-react-dom-components-common.md#common-props) like `onClick`. However, when you're building your own components, you can name their event handler props any way that you like.
...
Unlike rendering functions, event handlers don't need to be [pure](learn-keeping-components-pure.md), so it's a great place to *change* something—for example, change an input's value in response to typing, or change a list in response to a button press. However, in order to change some information, you first need some way to store it. In React, this is done by using [state, a component's memory.](learn-state-a-components-memory.md) You will learn all about it on the next page.
