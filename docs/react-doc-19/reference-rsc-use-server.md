...
`'use server'` is for use with [using React Server Components](reference-rsc-server-components.md).
...
Add `'use server'` at the top of an async function body to mark the function as callable by the client. We call these functions [_Server Functions_](reference-rsc-server-functions.md).
...
* To import a Server Functions from [client code](reference-rsc-use-client.md), the directive must be used on a module level.
...
* Server Functions should be called in a [Transition](reference-react-useTransition.md). Server Functions passed to [`<form action>`](reference-react-dom-components-form.md#props) or [`formAction`](reference-react-dom-components-input.md#props) will automatically be called in a transition.
...
See [experimental_taintUniqueValue](reference-react-experimental_taintUniqueValue.md) and [experimental_taintObjectReference](reference-react-experimental_taintObjectReference.md).
...
* React elements, or [JSX](learn-writing-markup-with-jsx.md)
...
Supported serializable return values are the same as [serializable props](reference-rsc-use-client.md#serializable-types) for a boundary Client Component.
...
To update the UI based on the result of a Server Function while supporting progressive enhancement, use [`useActionState`](reference-react-useActionState.md).
...
Note that like most Hooks, `useActionState` can only be called in <CodeStep step={1}>[client code](reference-rsc-use-client.md)</CodeStep>.
...
When using a Server Function outside a [form](reference-react-dom-components-form.md), call the Server Function in a [Transition](reference-react-useTransition.md), which allows you to display a loading indicator, show [optimistic state updates](reference-react-useOptimistic.md), and handle unexpected errors. Forms will automatically wrap Server Functions in transitions.
