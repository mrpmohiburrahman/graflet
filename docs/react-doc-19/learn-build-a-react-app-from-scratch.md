...
For a list of recommended frameworks, check out [Creating a React App](learn-creating-a-react-app.md).
...
Vite is already being used as a build tool in one of our [recommended frameworks](learn-creating-a-react-app.md): [React Router](https://reactrouter.com/start/framework/installation).
...
Similarly, if you rely on the apps using your framework to split the code, you might encounter situations where loading becomes slower than if no code splitting were happening at all. For example, [lazily loading](reference-react-lazy.md) a chart delays sending the code needed to render the chart, splitting the chart code from the rest of the app. [Parcel supports code splitting with React.lazy](https://parceljs.org/recipes/react/#code-splitting). However, if the chart loads its data *after* it has been initially rendered you are now waiting twice. This is a waterfall: rather than fetching the data for the chart and sending the code to render it simultaneously, you must wait for each step to complete one after the other.
...
If you don't want to solve these problems on your own, you can [get started with a framework](learn-creating-a-react-app.md) that provides these features out of the box.
