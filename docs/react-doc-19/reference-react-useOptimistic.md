...
* `optimisticState`: The value that you want the optimistic state to be during an [Action](reference/react/useTransition#functions-called-in-starttransition-are-called-actions). If you provided a `reducer` to `useOptimistic`, this value will be passed as the second argument to your reducer. It can be a value of any type.
    * If you pass a function as `optimisticState`, it will be treated as an _updater function_. It must be pure, should take the pending state as its only argument, and should return the next optimistic state. React will put your updater function in a queue and re-render your component. During the next render, React will calculate the next state by applying the queued updaters to the previous state similar to [`useState` updaters](reference-react-useState.md#setstate-parameters).
...
When using [Action props](reference-react-useTransition.md#exposing-action-props-from-components), you can call the set function without `startTransition`:
...
In an [Action prop](reference-react-useTransition.md#exposing-action-props-from-components), you can call the optimistic setter directly without `startTransition`.
...
See [Exposing `action` prop from components](reference-react-useTransition.md#exposing-action-props-from-components) for the Action prop pattern.
...
When creating an [Action prop](reference-react-useTransition.md#exposing-action-props-from-components), you can add `useOptimistic` to show immediate feedback.
...
You can also use [`useTransition`](reference-react-useTransition.md) to get pending state via `isPending`.
...
**Updater functions** work like [useState updaters](reference-react-useState.md#updating-state-based-on-the-previous-state). Pass a function to the setter:
...
The reducer handles three `action` types (`add`, `remove`, `update_quantity`) and returns the new optimistic state for each. Each `action` sets a `pending: true` flag so you can show visual feedback while the [Server Function](reference-rsc-server-functions.md) runs.
