...
`use` is a React API that lets you read the value of a [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise) or [context](learn-passing-data-deeply-with-context.md).
...
Call `use` with a [context](learn-passing-data-deeply-with-context.md) to read its value. Unlike [`useContext`](reference-react-useContext.md), `use` can be called within loops and conditional statements like `if`.
...
* `context`: A [context](learn-passing-data-deeply-with-context.md) created with [`createContext`](reference-react-createContext.md).

#### Returns {/*context-returns*/}

The context value for the passed context, determined by the closest context provider above the calling component. If there is no provider, the returned value is the `defaultValue` passed to [`createContext`](reference-react-createContext.md).
...
If the component that calls `use` is wrapped in a [Suspense](reference-react-Suspense.md) boundary, the fallback will be displayed while the Promise is pending. Once the Promise is resolved, the Suspense fallback is replaced by the rendered components using the data returned by `use`. If the Promise is rejected, the fallback of the nearest [Error Boundary](reference-react-Component.md#catching-rendering-errors-with-an-error-boundary) will be displayed.
...
* When passing a Promise from a Server Component to a Client Component, its resolved value must be [serializable](reference-rsc-use-client.md#serializable-types).
...
When a [context](learn-passing-data-deeply-with-context.md) is passed to `use`, it works similarly to [`useContext`](reference-react-useContext.md). While `useContext` must be called at the top level of your component, `use` can be called inside conditionals like `if` and loops like `for`.
...
Unlike [`useContext`](reference-react-useContext.md), <CodeStep step={2}>`use`</CodeStep> can be called in conditionals and loops like <CodeStep step={1}>`if`</CodeStep>.
...
Reading the value requires two `use` calls because the context value itself isn't awaited. See [Before you use context](learn-passing-data-deeply-with-context.md#before-you-use-context) for alternatives to consider before reaching for context.

Wrap the components that read the Promise in a [Suspense](reference-react-Suspense.md) boundary so only that subtree suspends while the Promise is pending. See [Usage (Promises)](#usage-promises) below for more on reading Promises with `use`.

<Pitfall>

When this pattern is used with [Server Components](reference-rsc-server-components.md), refetching the Promise requires refetching the Server Component that sets the Promise in context. Avoid setting the Promise in context high in the tree, since that would refetch large parts of the app unnecessarily.
...
Call `use` with a Promise to read its resolved value. The component will [suspend](reference-react-Suspense.md) while the Promise is pending.
...
Wrap the component that calls <CodeStep step={1}>`use`</CodeStep> in a [Suspense](reference-react-Suspense.md) boundary so React can show a fallback while the Promise is pending. The closest Suspense boundary above the suspending component shows its fallback. Once the Promise resolves, React reads the value with `use` and replaces the fallback with the rendered component.
...
In this example, `Albums` calls `use` with a cached Promise. The component suspends while the Promise is pending, and React displays the nearest Suspense fallback. Rejected Promises propagate to the nearest [Error Boundary](reference-react-Component.md#catching-rendering-errors-with-an-error-boundary).
...
Instead, pass a Promise from a cache, a [Suspense-enabled framework](reference-react-Suspense.md#suspense-enabled-frameworks), or a Server Component:
...
[React doesn't preserve state for renders that suspended before mounting](reference-react-Suspense.md#caveats). After each suspension, React retries rendering from scratch, so any Promise created during render is recreated.
...
To refresh data at the same URL (for example, with a "Refresh" button), invalidate the cache entry and start a new fetch inside a [`startTransition`](reference-react-startTransition.md). Store the resulting Promise in state to trigger a re-render. While the new Promise is pending, React keeps showing the existing content because the update is inside a Transition.
...
```
Because `Message` is wrapped in a [Suspense](reference-react-Suspense.md) boundary, the fallback will be displayed until the Promise is resolved. When the Promise is resolved, the value will be read by the `use` API and the `Message` component will replace the Suspense fallback.
...
See [Revealing content together at once](reference-react-Suspense.md#revealing-content-together-at-once) for guidance on where to place Suspense boundaries.
...
If the Promise passed to `use` is rejected, the error propagates to the nearest [Error Boundary](reference-react-Component.md#catching-rendering-errors-with-an-error-boundary). Wrap the component that calls `use` in an Error Boundary to display a fallback when the Promise is rejected.
