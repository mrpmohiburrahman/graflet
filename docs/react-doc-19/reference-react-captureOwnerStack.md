...
- React error handlers ([React Root options](reference-react-dom-client-createRoot.md#parameters) `onCaughtError`, `onRecoverableError`, and `onUncaughtError`)
...
The Owner Stack is different from the Component Stack available in React error handlers like [`errorInfo.componentStack` in `onUncaughtError`](reference-react-dom-client-hydrateRoot.md#error-logging-in-production).
