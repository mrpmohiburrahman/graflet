...
`<style>` supports all [common element props.](reference-react-dom-components-common.md#common-props)
...
React can move `<style>` components to the document's `<head>`, de-duplicate identical stylesheets, and [suspend](reference-react-Suspense.md) while the stylesheet is loading.
