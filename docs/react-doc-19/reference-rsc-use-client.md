...
`'use client'` is for use with [React Server Components](reference-rsc-server-components.md).
...
When a file marked with `'use client'` is imported from a Server Component, [compatible bundlers](learn-creating-a-react-app.md#full-stack-frameworks) will treat the module import as a boundary between server-run and client-run code.
...
* Functions that are [Server Functions](reference-rsc-server-functions.md)
* Client or Server Component elements (JSX)
* [Promises](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)

Notably, these are not supported:
* [Functions](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Function) that are not exported from client-marked modules or marked with [`'use server'`](reference-rsc-use-server.md)
...
* [`react`](reference-react-hooks.md) and [`react-dom`](reference-react-dom-hooks.md) Hooks, excluding [`use`](reference-react-use.md) and [`useId`](reference-react-useId.md)
