...
You could think of your components as recipes: if you follow them and don't introduce new ingredients during the cooking process, you will get the same dish every time. That "dish" is the JSX that the component serves to React to [render.](learn-render-and-commit.md)
...
You can fix this component by [passing `guest` as a prop instead](learn-passing-props-to-a-component.md):
...
Although you might not have used them all yet, in React there are three kinds of inputs that you can read while rendering: [props](learn-passing-props-to-a-component.md), [state](learn-state-a-components-memory.md), and [context.](learn-passing-data-deeply-with-context.md) You should always treat these inputs as read-only.

When you want to *change* something in response to user input, you should [set state](learn-state-a-components-memory.md) instead of writing to a variable. You should never change preexisting variables or objects while your component is rendering.
...
In React, **side effects usually belong inside [event handlers.](learn-responding-to-events.md)** Event handlers are functions that React runs when you perform some action—for example, when you click a button. Even though event handlers are defined *inside* your component, they don't run *during* rendering! **So event handlers don't need to be pure.**

If you've exhausted all other options and can't find the right event handler for your side effect, you can still attach it to your returned JSX with a [`useEffect`](reference-react-useEffect.md) call in your component. This tells React to execute it later, after rendering, when side effects are allowed. **However, this approach should be your last resort.**
...
* You can improve performance by [skipping rendering](reference-react-memo.md) components whose inputs have not changed. This is safe because pure functions always return the same results, so they are safe to cache.
...
* You should not mutate any of the inputs that your components use for rendering. That includes props, state, and context. To update the screen, ["set" state](learn-state-a-components-memory.md) instead of mutating preexisting objects.
