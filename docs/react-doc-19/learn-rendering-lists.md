...
The short [`<>...</>` Fragment](reference-react-Fragment.md) syntax won't let you pass a key, so you need to either group them into a single `<div>`, or use the slightly longer and [more explicit `<Fragment>` syntax:](reference-react-Fragment.md#rendering-a-list-of-fragments)
