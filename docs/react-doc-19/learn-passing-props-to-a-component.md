...
If double curly braces after `person=` confuse you, recall [they're merely an object](learn-javascript-in-jsx-with-curly-braces.md#using-double-curlies-css-and-other-objects-in-jsx) inside the JSX curlies.
...
The `Clock` component below receives two props from its parent component: `color` and `time`. (The parent component's code is omitted because it uses [state](learn-state-a-components-memory.md), which we won't dive into just yet.)
...
**Don't try to "change props".** When you need to respond to the user input (like changing the selected color), you will need to "set state", which you can learn about in [State: A Component's Memory.](learn-state-a-components-memory.md)
