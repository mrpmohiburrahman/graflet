...
`cache` is only for use with [React Server Components](reference-rsc-server-components.md).
...
- `cache` is for use in [Server Components](reference-rsc-server-components.md) only.
...
To render components that use asynchronous data in Client Components, see [`use()` documentation](reference-react-use.md).
...
#### When should I use `cache`, [`memo`](reference-react-memo.md), or [`useMemo`](reference-react-useMemo.md)? {/*cache-memo-usememo*/}
...
In general, you should use [`useMemo`](reference-react-useMemo.md) for caching an expensive computation in a Client Component across renders. As an example, to memoize a transformation of data within a component.
