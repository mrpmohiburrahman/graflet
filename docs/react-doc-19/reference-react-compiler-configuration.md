...
* [`compilationMode`](reference-react-compiler-compilationMode.md) controls the strategy for selecting functions to compile (e.g., all functions, only annotated ones, or intelligent detection).
...
[`target`](reference-react-compiler-target.md) specifies which React version you're using (17, 18, or 19).
...
These options control how the compiler responds to code that doesn't follow the [Rules of React](reference-rules.md).

[`panicThreshold`](reference-react-compiler-panicThreshold.md) determines whether to fail the build or skip problematic components.
...
[`logger`](reference-react-compiler-logger.md) provides custom logging for compilation events.
...
[`gating`](reference-react-compiler-gating.md) enables runtime feature flags for A/B testing or gradual rollouts.
