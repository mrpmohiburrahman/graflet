...
- [x] "Sign in with GitHub" runs the backend's OAuth code-exchange (docs-kg-cli ticket 03); the client secret appears in no shipped frontend asset or browser network payload (grep/DevTools verifiable, ADR-0001). — exchange runs in `/auth/web/callback`; frontend only navigates to `/auth/web/start`; test asserts the authorize URL carries no secret.
