...
- [ ] A public Privacy page discloses the stored fields (`github_id`, email, `marketing_consent`), the tri-state consent model, and the withdraw/unsubscribe path — consistent with ADR-0006.
- [ ] A public Terms page states free + open-source, as-is, no warranty, and links the OSS repo/license.
- [ ] An attribution page lists each redistributed doc's `repo_url` + `license_id` from the catalog (reusing ticket 04's catalog client), satisfying the green-license redistribution terms.
- [ ] All three pages are linked from the footer and load with no sign-in and no data capture; the unsubscribe path referenced by notification emails is reachable from Privacy.
